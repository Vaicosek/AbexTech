...
class MoneyCog(commands.Cog):
    ...
    async def futures_order(self,
        ...
        if channel is not None:
            # THE TITLE IS PARSED. `views/web.py::FuturesOrderView._oid` recovers
            # this order's id after a restart with `re.search(r"#(\d+)", embed.title)`
            # when the message-id lookup misses, and cards posted before today are
            # still in #futures with live Approve/Decline buttons. `#<id>` stays in
            # the title, in that shape, whatever else the title says.
            embed = ab.embed(
                title=f"Futures order #{order_id}",
                kicker="/futures",
                desc=f"{interaction.user.mention} ordered "
                     f"**{quantity} × {item}**. Awaiting owner review — nothing is "
                     f"charged and nothing is produced until it is approved.",
                groups=[("Enchants / quality", enchants or ""),
                        ("Notes", notes or "")],
                colour=ab.ACCENT,
            )
