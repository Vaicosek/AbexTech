...
ACCOUNT_LABELS = {
    ...
    "treasury:vtech": "Abex Tech house account",
}
...
def _since(created_at) -> str:
    """A split run's `created_at` (unix float) as Discord's own relative stamp.

    `parked {mins} min` was computed here from `time.time()`, which is right for
    exactly one reader in one timezone at one moment; `<t:unix:R>` is right for
    everyone and keeps counting while the card sits on screen.
    """
    try:
        return ab.when(_dt.datetime.fromtimestamp(float(created_at),
                                                  _dt.timezone.utc))
    except Exception:  # noqa: BLE001
        # An unreadable timestamp falls back to what the card said before, not to
        # an em-dash: "parked — · attempts 3" tells a manager nothing, and the row
        # it appears on is a stuck split run somebody has to act on.
...
def build_change_embed(src: str, before: list[tuple[str, int]],
    ...
    `what` is the description — the one sentence naming what this write does —
    and `warning`, when there is one, is the thing a manager must read before
    pressing, so it sits directly under the table rather than in a footer. The
    side-by-side block stays monospace: it is a real table and it has to line up.
    """
    return ab.embed(
        title=title,
        kicker="/splits",
        desc=what,
        groups=[
            (f"Where {_source_label(src)} income goes",
             _change_table(before, after)[:1024]),
            ("Short-source policy",
             (f"**{policy[0]}** -> **{policy[1]}**"
              if policy and policy[0] != policy[1] else "")),
            ("Read this before you confirm", warning[:1024] if warning else ""),
            ("What this does to money already in flight", _PIN_NOTE),
        ],
        foot="Nothing has been written yet. This is a preview.",
        colour=ab.LOSS if warning else ab.ACCENT,
    )
...
class SplitsCog(commands.Cog):
    ...
    splits = app_commands.Group(
        ...
        # Discord hides this from non-managers, so a new member never sees it in the
        # picker. The body still calls core.is_manager(), so this is display only —
        # it removes confusion, not a permission check.
    ...
    async def splits_list(self, interaction: discord.Interaction,
        ...
        desc = f"`{src}` · ruleset **v{rs['version']}**"
        if not rs["rules"]:
            # An empty account is a sentence, not a field with a dash in it.
            desc += ("\nNo rules — nothing is routed. This account behaves exactly "
                     "as it did before the split engine existed: the commission "
                     "stays where it lands and no split run is ever minted. "
                     "`/splits add` turns it on.")
        groups = []
        for position, r in enumerate(rs["rules"], start=1):
            ...
            if int(r["floor_coins"] or 0):
                bits.append(f"skipped below {ab.coins(int(r['floor_coins']))}")
            bits.append(f"{ab.coins((SAMPLE_INCOME * int(r['bps'])) // 10000)} per "
                        f"{ab.coins(SAMPLE_INCOME)} of income")
            groups.append((f"{position}. #{r['id']} · {_pct(r['bps'])}",
                           "\n".join(bits)[:1024]))
        emb = ab.embed(
            title=f"Split rules · {ACCOUNT_LABELS.get(src, src)}",
            kicker="/splits list",
            desc=desc,
            band=[("Allocated", _pct(rs["total_bps"])),
                  ("Retained", _pct(rs["retained_bps"])),
                  ("Short policy", str(rs["short_policy"]))],
            groups=groups,
            foot="Order matters: under the `prorate` policy the LAST rule "
    ...
    async def splits_runs(self, interaction: discord.Interaction,
        ...
        The three states used to be told apart by an emoji on each field name.
        They are told apart by the word that names them now — `Stuck`, `Waiting
        for coins`, `Unrouted` — which is the same distinction and survives
        being read aloud, quoted, or searched for.
        """
        if not _is_staff(interaction):
            return await _deny(interaction)
        await interaction.response.defer(ephemeral=True)
        ...
        for r in stuck[:10]:
            groups.append((
                f"Stuck · {r['state']} · {ab.coins(int(r['amount_in']))} from "
                f"{_acct(r['source_account'])}",
                f"`{r['run_id']}`\nattempts {r['attempts']} · "
                f"{r['reason'] or 'no reason recorded'}"))
        for r in parked[:10]:
            groups.append((
                f"Waiting for coins · {ab.coins(int(r['amount_in']))} from "
                f"{_acct(r['source_account'])}",
                f"`{r['run_id']}`\n{r['trigger_kind']} #{r['trigger_row_id']} · "
                f"parked {_since(r['created_at'])} · attempts {r['attempts']}\n"
                f"It pays its PINNED plan the moment the source can fund it — top "
                f"the account up, or `/splits run` to see exactly who it will pay."))
        for r in unrouted[:10]:
            groups.append((
                f"Unrouted · {ab.coins(int(r['amount_in']))} from "
                f"{_acct(r['source_account'])}",
                f"`{r['run_id']}`\n{r['trigger_kind']} #{r['trigger_row_id']} · "
                f"refused: {r['reason'] or 'no reason recorded'}\n"
                f"**No coins moved** — the {ab.coins(int(r['amount_in']))} is still "
                f"in {_acct(r['source_account'])}. Fix what refused it "
                f"(`/splits list`), and the NEXT offer of this event plans it again "
                f"from scratch; a settled lot is offered once, so this one needs you."))

        emb = ab.embed(
            title="Split runs a human should look at",
            kicker="/splits runs",
            desc=(f"{len(stuck)} stuck · {len(parked)} parked · "
                  f"{len(unrouted)} unrouted."),
            groups=groups,
            foot="Stuck needs a human. Waiting for coins is the `defer` policy "
                 "working — unless nobody is going to top that account up. "
                 "Unrouted refused: nothing moved, nobody was paid.",
            colour=ab.LOSS if (stuck or unrouted) else ab.NEUTRAL,
        )
        await interaction.followup.send(embed=emb, ephemeral=True)
    ...
    async def splits_run(self, interaction: discord.Interaction, run_id: str):
        ...
        emb = ab.embed(
            ...
            kicker="/splits run",
            desc=(f"`{row['run_id']}`\nsource {_source_label(row['source_account'])} "
                  f"· ruleset v{row['ruleset_version']} · policy "
                  f"{row['short_policy']} · attempts {row['attempts']}"),
            band=[("In", ab.coins(int(row["amount_in"]))),
                  ("Allocated", ab.coins(int(row["allocated"]))),
                  ("Shortfall", ab.coins(int(row["shortfall_coins"])))],
            groups=[
                ("Reason", str(row["reason"])[:1000] if row.get("reason") else ""),
                (f"{len(legs)} leg(s)" if legs else "",
                 "\n".join(
                     f"{_display(l['kind'], l['to_account'], guild=guild)} "
                     f"— {ab.coins(int(l['amount']))} · {l['state']}"
                     for l in legs[:20])[:1024]),
            ],
            foot="A run pins its plan. These legs are what it will pay, "
                 "whatever the rules say now.",
            colour=ab.ACCENT if row["state"] == "applied" else ab.LOSS,
        )
        await interaction.followup.send(embed=emb, ephemeral=True)
