...
class ValuationCog(commands.Cog):
    ...
    async def list_public(self, interaction: discord.Interaction, market_id: str, force: bool = False):
        ...
        embed = ab.embed(
            ...
            kicker="/list_public",
            desc=(f"Listed at {ab.coins(price)} per share, {shares:,.0f} shares "
                  f"outstanding. Grade **{b['grade']}** — {b['pct_of_cap']:.0f}% of "
                  f"the cap is backed by assets."),
            band=[("Share price", f"{ab.coins(price)} per share"),
                  ("Shares", f"{shares:,.0f}"),
                  ("Market cap", ab.coins(v["market_cap"]))],
            groups=[
                ("Set from the model", ab.rows([
                    ("asset_value", f"{ab.coins(v['market_cap'])} (pins price)"),
                    ("sellable", ab.coins(sellable)),
                    ("treasury", ab.coins(b["cash"])),
                ])),
                ("Earnings per month", ab.rows([
                    ("CSN", ab.coins(_e["csn_trailing_avg"])),
                    ("Hive", ab.coins(_e["hive_income_site_net"])),
                    ("Transport fees", ab.coins(_e["tp_fee_income"])),
                ])),
                ("Must fix, or the quote drifts",
                 "\n".join(f"• {a[len('CRITICAL: '):]}" for a in crit)[:1000]
                 if crit else ""),
            ],
            foot=f"Trade {market_id} on the dashboard exchange · delist from /my market",
            colour=ab.ACCENT,
        )
