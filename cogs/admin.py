...
def _pieces_and_stacks(qty: int) -> str:
    """`4,608 pieces (72 stacks)` — and only when it divides exactly.

    A floored stack count printed beside the piece count reads as an equivalence:
    "1,000 pieces (15 stacks of 64)" is short by 40 pieces. When it does not
    divide, the piece count stands alone.
    """
    qty = int(qty or 0)
    stacks, remainder = divmod(qty, 64)
    if stacks and not remainder:
        return f"{qty:,} pieces ({stacks:,} stacks of 64)"
    return f"{qty:,} pieces"



#: A closing post is found again by its title. The card used to be headed
#: "📕 Month closed — <market> · <month>"; the house builder strips the marker and
#: the em-dash, so it now reads "Month closed · <market> · <month>". Posts made
#: before that change are still sitting in report channels and still have to be
#: findable, so every reader matches BOTH shapes. Drop the old one only when no
#: pre-change post can still exist.
_CLOSING_TITLE_PREFIXES = ("Month closed", "\U0001F4D5 Month closed")


def _is_closing_title(title: str) -> bool:
    return str(title or "").lstrip().startswith(_CLOSING_TITLE_PREFIXES)
...
class AdminCog(commands.Cog):
    ...
    def _hive_month_embed(self, site_name, mid, mk, md):
        """One hive site's month, in the house shape.

        The figures are unchanged — `value`, `paid_value`, `qty` and the two
        breakdowns come from the stored month exactly as before. What changed is
        that the harvest quantity now says what unit it is in (pieces, and the
        same number in stacks of 64, which is how anyone actually carries it),
        and the coin figures carry `c` instead of a coin emoji. Every figure is
        truncated with int() before formatting, exactly as the old card did.

        The two tables stay code blocks: they are columns that have to line up,
        and monospace is the one place a table is right.
        """
        ...
        desc = (f"{len(igns)} harvester(s) across {len(items)} item type(s). "
                + (f"{ab.coins(int(owed))} is still owed to them."
                   if owed >= 1 else "Everything harvested has been paid out."))

        groups = []
        top = sorted(igns.items(), key=lambda kv: -kv[1]["value"])[:10]
        if top:
            w = max(len(n[:16]) for n, _ in top)
            rows = [f"{i+1:>2}. {n[:16]:<{w}} {v['qty']:>6,}x {int(v['value']):>9,}c"
                    for i, (n, v) in enumerate(top)]
            groups.append(("Harvesters", "```\n" + "\n".join(rows) + "\n```"))

        ti = sorted(items.items(), key=lambda kv: -kv[1]["value"])[:6]
        if ti:
            w = max(len(core._pretty_item_name(n)[:18]) for n, _ in ti)
            rows = [f"{core._pretty_item_name(n)[:18]:<{w}} {v['qty']:>6,}x {int(v['value']):>9,}c"
                    for n, v in ti]
            groups.append(("Harvested", "```\n" + "\n".join(rows) + "\n```"))

        return ab.embed(
            title=f"{site_name} · {mk}",
            kicker="Hive month",
            desc=desc,
            band=[("Harvested", _pieces_and_stacks(qty)),
                  # int() BEFORE formatting, as the old card did. `:,.0f` rounds,
                  # and every one of the 388 rows in hive_harvests carries a
                  # fractional unit_value — bnl 2026-08 moved 63,989 -> 63,990c
                  # purely from restyling. A card that changes a figure is not a
                  # restyle.
                  ("Worth", ab.coins(int(value))),
                  ("Paid out", ab.coins(int(paid)))],
            groups=groups,
            foot=f"Hive site · {site_name}",
            # An unpaid balance is the one thing on this card that needs acting
            # on, and Warm Feel has one token for "needs attention or money out".
            colour=ab.LOSS if owed >= 1 else ab.ACCENT,
        )
    ...
    async def fix_month_close(self, month=None, market_id=None, repost: bool = False) -> str:
        ...
        for mid, month in jobs:
                ...
                try:
                    async for msg in channel.history(limit=100):
                        if (msg.author.id == self.bot.user.id and msg.embeds
                                and _is_closing_title(msg.embeds[0].title)
            ...
            spent = float(md.get("spent", 0) or 0)
            name = m.get("name", mid)
            # Same shape the house builder produces, so a card posted here and a
            # card posted through `_build_csn_embed` are found by the same reader.
