...
def _closes_line(closes_at) -> str:
    """`Closes <t:unix:R>` — Discord renders it in each reader's own timezone.

    The footer used to read `closes {closes_at} UTC` straight out of the row,
    which is only right for a reader who does the arithmetic themselves. A value
    that will not parse falls back to exactly the old text rather than to a
    guessed time.
    """
    s = str(closes_at or "").strip()
    if not s:
        return ""
    for fmt, n in (("%Y-%m-%d %H:%M:%S", 19), ("%Y-%m-%dT%H:%M:%S", 19),
        ...
        try:
            ...
            return f"Closes {ab.when(dt.replace(tzinfo=timezone.utc))}."
    ...
    try:
        ...
        return f"Closes {ab.when(dt)}."
    except Exception:  # noqa: BLE001
        return f"Closes {s} UTC."
...
def _results_embed(p: dict, final: bool = False) -> discord.Embed:
    ...
    for i, o in enumerate(opts):
        ...
        # The winner is named in a word rather than marked with a trophy: these
        # standings are read on a phone by people deciding how to vote.
        mark = "  (winner)" if (final and i == top and tally[i] > 0) else ""
        lines.append(f"**{o}**{mark}\n`{bar:<20}` {pct:5.1f}%  ({tally[i]:,.0f})")

    closed = final or p.get("status") != "open"
    desc = "\n".join(lines)
    if not closed:
        closes = _closes_line(p.get("closes_at"))
        if closes:
            desc = (desc + "\n\n" + closes) if desc else closes

    return ab.embed(
        title=f"#{p['id']} {p.get('question', '')}"[:256],
        kicker="Final result" if final else "Standings",
        desc=desc,
        band=[("Voters", f"{len(votes):,}"),
              ("Total weight", f"{sum(tally):,.0f}")],
        foot=f"{p.get('market_id', '?')} holders · " + ("closed" if closed else "open"),
        colour=ab.ACCENT if final else ab.NEUTRAL,
    )
