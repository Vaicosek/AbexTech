...
import abex_embed as ab  # noqa: E402
...
def _stamp(value) -> str:
    """An audit row's `created_at` / `finished_at` as Discord's own stamp.

    The rows carry ISO-ish UTC text out of SQLite, and the old rendering was
    `str(value)[:19] + " UTC"` — correct, and wrong for every reader who is not
    on UTC. `<t:unix:R>` is the same instant in the reader's own clock. A string
    that will not parse falls back to exactly what was printed before, because
    inventing a time on an audit row is worse than printing the stored text.
    """
...
def _kind_label(kind) -> str:
    """`land_unwind` -> `Land unwind`. The stored `kind` is an internal id.

    `action_log.record` is called with slugs like 'land_unwind' and 'land_sale';
    putting one straight into the kicker prints an id on a card a manager reads.
    Unknown slugs still render, just as words.
    """
...
def build_action_embed(action: dict, *, guild=None) -> discord.Embed:
    ...
    House shape (`abex_embed`): the audit kind is the kicker, the summary is the
    title, and the ONE sentence a reader must not miss — who reversed it, or that
    the coins are theirs to move by hand — is the description rather than a
    footer. `By / When / Money moved` are the three band tiles.
    """
    state = action.get("state") or "open"
    colour = {"open": ab.NEUTRAL, "claimed": ab.NEUTRAL, "done": ab.ACCENT,
              "partial": ab.LOSS, "failed": ab.LOSS}.get(state, ab.NEUTRAL)

    band = []
    if action.get("actor_name"):
        band.append(("By", action["actor_name"]))
    stamp = _stamp(action.get("created_at"))
    if stamp:
        band.append(("When", stamp))
    coins = int(action.get("money_coins") or 0)
    if coins:
        band.append(("Money moved", ab.coins(coins)))
    ...
    if pv["lines"]:
        ...
        # The caveat is said TWICE on purpose: once in the sentence at the top,
        # where a reader who stops after the title still sees it, and once in the
        # field name, so it survives being read out of order. A reader must not
        # walk away thinking the coins come back on their own.
        groups.append((("Undo would (coins by hand — see below)"
                        if by_hand else "Rollback would"), body[:1024]))
    else:
        # Kept in its own variable, not in `desc`: the state chain below reassigns
        # desc, and a card for a claimed-or-done row was silently dropping the one
        # sentence saying the coins do NOT come back on their own.
        not_reversible = ("Not automatically reversible — the button opens a staff "
                          "task instead of pretending to undo it.")
        desc = not_reversible

    if state == "claimed":
        who = action.get("claimed_name") or action.get("claimed_by") or "staff"
        desc = f"{who} is rolling this back right now."
    elif state in ("done", "partial", "failed"):
        who = action.get("claimed_name") or action.get("claimed_by") or "staff"
        fin = _stamp(action.get("finished_at"))
        if state == "failed":
            desc = f"The rollback by {who} failed part-way — see the staff task."
        elif state == "partial":
            desc = (f"Rolled back in part by {who} — the rest is on a staff task.")
        else:
            desc = f"Rolled back by {who}" + (f" {fin}" if fin else "") + "."
    elif by_hand:
        desc = (f"Undoing this reverses the record only: {ab.coins(by_hand)} "
                f"would stay where they are and must be moved by hand.")

    # If the row is not automatically reversible, that outranks a status line: a
    # reader who sees only "Rolled back by staff" walks away believing the coins
    # moved back.
    if not_reversible and not_reversible not in desc:
        desc = f"{desc} {not_reversible}".strip()

    return ab.embed(
        title=str(action["summary"])[:256],
        kicker=_kind_label(action.get("kind")),
        desc=desc,
        band=band,
        groups=groups,
        colour=colour,
...
def build_confirm_embed(action: dict, pv: dict) -> discord.Embed:
    ...
    move; it is the DESCRIPTION here, because the sentence that decides whether
    a manager should press the button belongs above the figures, not in a field
    a third of the way down a phone screen.

    The fixed-width table stays a code block: monospace is the one place a table
    is right, and these columns have to line up to be read at all.
    """
    coins = sum(abs(int(m[2])) for m in pv["movements"])
    by_hand = int(pv.get("manual_coins") or 0)
    short_total = sum(abs(int(m[4])) for m in pv["movements"])

    lines = [f"**{action['summary']}**"]
    if by_hand:
        lines.append(
            f"This button does NOT move {ab.coins(by_hand)}. It reverses the "
            f"**record**: those coins stay where they are and must be moved by a "
            f"human, with the exact steps and figures in the staff task below. "
            f"Do not press it expecting a refund to go out.")
    elif coins:
        lines.append(f"Confirming moves {ab.coins(coins)} back, in the legs below.")

    groups = []
    if pv["movements"]:
        groups.append((f"Coins this button will move: {ab.coins(coins)}",
                       _figures_block(pv["movements"])[:1024]))
    other = [ln for ln in pv["lines"] if not ln.lstrip("+-").split(" ")[0].replace(",", "").isdigit()]
    if other:
        groups.append(("Also", "\n".join(f"• {o}" for o in other[:8])[:1024]))
    if short_total:
        groups.append(("Cannot fully claw back",
                       f"{ab.coins(short_total)} have already been spent. That part "
                       f"will open a staff task rather than drive a balance negative."))
    if pv["manual"]:
        groups.append(("Needs a human",
                       "\n".join(f"• {m}" for m in pv["manual"][:5])[:1024]))
    if pv["already_done"]:
        groups.append(("Resuming",
                       f"{pv['already_done']} step(s) already applied by an earlier "
                       f"attempt will be skipped."))
    if pv.get("stale_claim"):
        who = action.get("claimed_name") or action.get("claimed_by") or "someone"
        groups.append((
            "Taking over a stalled rollback",
            f"**{who}** claimed this {_stamp(action.get('claimed_at')) or 'earlier'} "
            f"and it never finished — most likely a restart mid-rollback. Confirming "
            f"takes the claim over. Every step whose money already committed is "
    ...
    return ab.embed(
        ...
        desc="\n\n".join(lines),
        groups=groups,
        foot="Nothing has moved yet. This is a preview.",
        colour=ab.LOSS if (by_hand or short_total or pv.get("stale_claim")) else ab.ACCENT,
    )
...
class StaffTaskView(discord.ui.View):
    ...
    async def done(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Green is reserved for money RECEIVED. A closed task is an official
        # statement about a record, so it takes the accent bar.
        e.colour = ab.ACCENT
...
async def post_staff_task(client, task_id: int, *, channel=None):
    ...
    if ch is None:
        return None
    # The footer MUST keep `Task #<id>` first: `_task_id_from_message` re-resolves
    # this card's row from it after a restart, and cards posted before today are
    # still in the ops channel with live buttons. Everything after the marker is
    # free text; the marker is not.
    e = ab.embed(
        title=str(t["title"])[:256],
        kicker="Staff task",
        desc=str(t["body"])[:4000],
        foot=f"Task #{t['id']} · opened because the rollback could not "
