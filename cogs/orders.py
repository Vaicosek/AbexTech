...
class OrdersCog(commands.Cog):
    ...
    async def manager_panel(self, interaction: discord.Interaction):
        ...
        embed = ab.embed(
            title="Manager panel",
            desc="Everything below acts on real orders and real coins.",
            groups=[("What each button does", ab.rows([
                ("View orders", "the full order list, same as `/orders`"),
                ("Escalate order", "repost or bump an order to workers"),
                ("Cancel order", "pick an open order and cancel it"),
                ("Fund project", "hand a manager a budget"),
                ("Pay from project", "pay their team out of it"),
                ("Prune cancelled",
                 "deletes cancelled orders — fulfilled ones are kept as history"),
            ]))],
            colour=ab.ACCENT,
        )
