...
import abex_embed as ab             # the house embed (Abex Tech, Warm Feel)
...
def _listing_embed(listing: dict, bids: Optional[list] = None) -> discord.Embed:
    """One lot, in the house shape.

    The old card put eight or nine INLINE fields on a phone: seller, category,
    chunks, coords, reserve, current bid, buy-it-now, ends, backs-company. At
    380px that is a stack of half-empty tiles with the price somewhere inside it.
    Here the three money figures are the band, everything descriptive is one
    `Details` block, and the sentence that decides what a reader can DO with the
    lot — frozen, sold, cancelled, reversed — is the description, above the
    buttons rather than below the fold.
    """
    status = listing["status"]
    colour = {"active": ab.ACCENT, "sold": ab.ACCENT, "expired": ab.NEUTRAL,
              "cancelled": ab.LOSS, "rolled_back": ab.LOSS}.get(status, ab.NEUTRAL)
    is_land = (listing.get("kind") or "item") == "land"

    said = []
    if (listing.get("description") or "").strip():
        said.append(listing["description"].strip()[:1500])
    # A frozen exchange says so WHERE THE PLAYER IS LOOKING, not only in the
    # refusal of an action they already tried. First sentence after the lot's own
    # description, so it is above the Bid button on every screen size.
    if status == LIVE_STATUS:
        try:
            ...
            _notice = ""
        if _notice:
            said.append(f"**{FREEZE_HEADING}** — {_notice}")
        if listing["mode"] == "auction" and listing.get("ends_at"):
            said.append(f"Ends <t:{_epoch(listing['ends_at'])}:R>.")
    elif status == "sold":
        said.append(f"Sold for {ab.coins(listing['sold_price'])} to "
                    f"<@{listing['sold_to']}>.")
    elif status == "expired":
        said.append("Ended with no winning bid.")
    elif status == "cancelled":
        said.append("Cancelled by the seller.")
        ...
        said.append("This sale was reversed by staff. The lot is closed and cannot "
                    "be bid on; the seller can put it up again with `/sell`. If "
                    "coins were involved, staff will confirm where they ended up — "
                    "ask them rather than assuming.")

    # The band is the money and nothing else — at most three, because four wrap.
    band = []
    if listing["mode"] == "auction":
        cur = listing.get("current_bid")
        band.append(("Current bid",
                     f"{ab.coins(cur)} · <@{listing['current_bidder']}>" if cur
                     else "No bids yet — be first"))
        band.append((("Starting / AI reserve" if is_land else "Starting price"),
                     ab.coins(listing["reserve"])))
        if listing.get("buy_now"):
            band.append(("Buy it now", ab.coins(listing["buy_now"])))
    elif listing.get("buy_now") is not None:
        band.append(("Price", ab.coins(listing["buy_now"])))

    details = [("Seller", f"<@{listing['seller_id']}>")]
    if listing.get("category"):
        details.append(("Category", f"`{listing['category']}`"))
    if is_land and listing.get("chunks"):
        details.append(("Chunks", _fmt(listing["chunks"])))
    if is_land and listing.get("coords"):
        details.append(("Coords", f"`{listing['coords']}`"))
    if is_land and listing.get("market_id"):
        details.append(("Backs company", f"`{listing['market_id']}`"))

    groups = [("Details", ab.rows(details))]
    if bids:
        groups.append(("Recent bids", "\n".join(
            f"{ab.coins(b['amount'])} — <@{b['bidder_id']}>" for b in bids[:5])))
    ...
    return ab.embed(
        title=f"{_listing_title(listing)} · #{listing['id']}",
        kicker="Land Exchange" if is_land else "Item exchange",
        desc="\n\n".join(said)[:4000],
        band=band,
        groups=groups,
        foot=tail,
...
def _gallery_embeds(listing: dict, bids: Optional[list] = None,
    ...
    out = [main]
    for u in imgs[1:4]:
        # NOT an abex_embed: these carry no title, no text and no colour bar —
        # they are image tiles that Discord stitches under `main` because they
        # share its url. A house embed here would add a heading to a photo.
...
class LandExchangeCog(commands.Cog):
    ...
    async def listings(self, interaction: discord.Interaction):
        ...
        groups = []
        for r in rows[:20]:
            price = ((f"bid {ab.coins(r['current_bid'])}" if r.get("current_bid")
                      else f"reserve {ab.coins(r['reserve'])}")
                     if r["mode"] == "auction" else ab.coins(r["buy_now"]))
            ends = f" · ends <t:{_epoch(r['ends_at'])}:R>" if r.get("ends_at") else ""
            name = f"#{r['id']} " + (r.get("land") or (r.get("description") or "Land")[:40])
            groups.append((name[:256],
                           f"{'Auction' if r['mode'] == 'auction' else 'Fixed price'} · "
                           f"{price} · {_fmt(r['chunks'])} chunks{ends}"))
        emb = ab.embed(
            title="Land Exchange — active listings",
            kicker="/realestate listings",
            desc=f"{len(rows)} lot(s) open, soonest-ending first.",
            groups=groups,
            foot=(f"+{len(rows) - 20} more active — use /realestate info for a "
                  f"specific listing" if len(rows) > 20 else ""),
            colour=ab.ACCENT,
        )
    ...
    async def notifypanel(self, interaction: discord.Interaction):
        ...
        embed = ab.embed(
            ...
            desc="Click a button to toggle a ping role for yourself. You'll be "
                 "@mentioned when a new listing of that type goes up — click again "
                 "any time to opt out.",
            colour=ab.ACCENT,
        )
    ...
    async def config(self, interaction: discord.Interaction, commission_pct: Optional[float] = None,
        ...
        desc = ""
        if state["frozen"]:
            # The DESCRIPTION, not a field and not a footer: while the switch is
            # on it is the only line that matters, and it carries who + when +
            # how to undo it.
            when = ""
            if state["at"]:
                try:
                    when = (" " + ab.when(datetime.strptime(state["at"], "%Y-%m-%d %H:%M:%S")
                                          .replace(tzinfo=timezone.utc)))
                except Exception:  # noqa: BLE001 — never invent a time on a kill switch
                    when = f" at `{state['at']}` UTC"
            desc = (f"**{FREEZE_HEADING} — new bids and buys are blocked.** "
                    f"Set by {state['by'] or 'unknown'}{when}"
                    + (f", reason: {state['reason']}" if state["reason"] else "")
                    + ". In-flight lots STILL settle — this stops money entering, "
                      "not completing. Re-open with "
                      "`/realestate config bidding_frozen:False`.")
        embed = ab.embed(
            title="Land Exchange configuration",
            kicker="/realestate config",
            desc=desc,
            groups=[("Settings",
                     ab.rows([(k, f"`{_gd(_db, k, DEF[k])}`") for k in DEF]))],
            colour=ab.LOSS if state["frozen"] else ab.ACCENT,
        )
