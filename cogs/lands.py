...
class LandsCog(commands.Cog):
    ...
    async def _ingest(self, message, content: str, unlocked: bool = False):
        ...
        if new_entries and verbose:
            try:
                # The unlocked-channel warning is the CONSEQUENCE of this card, so
                # it is the first line of the description instead of a separate
                # message above the embed that scrolls away on its own.
                warn = ("**No lands-feed channel is locked** — this was accepted "
                        "from **any** webhook, including ones belonging to other "
                        "markets. Ask the bot to lock the feed channel to restrict "
                        "ingestion to your official feed channel and close this "
                        "gap.") if unlocked else ""
                body = "\n".join(report)
                emb = ab.embed(
                    title="Lands feed",
                    kicker="Feed ingest",
                    desc=((warn + "\n\n" if warn else "") + body)[:3900],
                    band=[("New ledger entries", f"{new_entries:,}"),
                          ("Balance snapshots", f"{len(balances):,}")],
                    colour=ab.LOSS if unlocked else ab.ACCENT)
