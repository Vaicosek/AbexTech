...
class MarketCog(commands.Cog):
    ...
    async def market_sales(self, interaction: discord.Interaction,
        ...
        groups, chunk, size, part = [], [], 0, 1
        for iname, v in rows:
            line = (f"{int(v.get('sold_qty', 0) or 0):,} × {iname} — "
                    f"{ab.coins(int(v.get('net_coins', 0) or 0))}")
            if size + len(line) + 1 > 1000 or len(chunk) >= 18:
                groups.append((f"Items ({part})", "\n".join(chunk)))
                chunk, size, part = [], 0, part + 1
                if part > 6:
                    break
            chunk.append(line); size += len(line) + 1
        if chunk and part <= 6:
            groups.append((f"Items ({part})" if part > 1 else "Items", "\n".join(chunk)))

        e = ab.embed(
            title=f"{markets[mid].get('name', mid)} — {md.get('label', mk)}",
            kicker="/market sales",
            band=[("Income", ab.coins(inc)), ("Spent", ab.coins(spent)),
                  ("Net", ab.coins(net))],
            groups=groups,
            foot=(f"{len(rows)} items total — full list on the dashboard."
                  if part > 6 else
                  f"{len(rows)} item(s) · {len(months)} month(s) on record"),
            # Red is a LOSS, not "a number below zero for decoration": a month
            # that finished under water is the one case this bar should be red.
