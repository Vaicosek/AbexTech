"""
admin_web.py — the owner's "god mode" for the Abex Tech site.
...
_CSS = """
<style>
/* Owner console. Same skin as everything else — being staff is not a theme. */
.adm-grid{display:grid;grid-template-columns:repeat(12,1fr);gap:34px}
.adm-tile{grid-column:span 12;border:none;border-top:1px solid var(--border);
  background:none;padding:20px 0 0}
.adm-tile.s6{grid-column:span 6}
.adm-h{font-size:28px;font-weight:400;color:var(--text);margin-bottom:10px;
  display:flex;align-items:center;gap:12px}
.adm-h::after{content:none}
.adm-empty{padding:6px 0;color:var(--faint);font-size:21px}
/* The kill switch is the one control that reads as dangerous, and it says so
   in words rather than in a filled badge. */
.adm-kill{display:flex;align-items:center;justify-content:space-between;gap:16px;
  border:none;border-left:3px solid var(--red);background:none;padding:4px 0 4px 16px}
.adm-kill .figs{font-size:21px;color:var(--text-body);display:block}
.adm-kill .figs b{color:var(--text);font-family:var(--font-data);
  font-variant-numeric:tabular-nums;font-weight:700}
.adm-state{font-family:var(--font-ui);font-size:21px;font-weight:700;padding:0}
.adm-state.on{background:none;color:var(--red)}
.adm-state.off{background:none;color:var(--faint);border:none}
.adm-form{display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;margin-top:8px}
.adm-form input{background:none;border:1px solid var(--border);color:var(--text);
  font-family:var(--font-data);font-size:21px;padding:9px 12px;min-width:280px}
.adm-form input:focus{outline:none;border-color:var(--accent)}
.adm-err{margin-top:9px;font-size:19px;color:var(--red)}
.adm-ok{margin-top:9px;font-size:19px;color:var(--green)}
.adm-num{font-family:var(--font-data);font-variant-numeric:tabular-nums;text-align:right}
.adm-sub{font-size:20px;color:var(--text-body);max-width:70ch;text-wrap:pretty}
@media (max-width:820px){.adm-tile.s6{grid-column:span 12}.adm-grid{gap:24px}}
</style>
"""
...
<style>
/* Sales log. */
.sl-chips{display:flex;flex-wrap:wrap;gap:22px;margin:20px 0}
.sl-chip{border:none;background:none;color:var(--text-body);font-size:21px;padding:0;
  text-decoration:none;font-family:var(--font-ui)}
.sl-chip:hover{color:var(--text)}
.sl-chip.on{color:var(--accent);text-decoration:underline;text-underline-offset:3px}
.sl-t{width:100%;border-collapse:collapse}
.sl-t th{text-align:left;font-size:21px;color:var(--faint);font-weight:400;
  padding:0 14px 10px 0;white-space:nowrap}
.sl-t td{padding:10px 14px 0 0;vertical-align:top;font-size:21px;
  border-top:1px solid var(--border)}
.sl-t tr.sl-x td{border-top:none;padding-top:2px;padding-bottom:14px}
.sl-when{font-family:var(--font-data);white-space:nowrap;color:var(--faint)}
.sl-unk{color:var(--red)}
.sl-kind{font-family:var(--font-ui);font-size:18px;color:var(--faint);white-space:nowrap}
.sl-kind.otc{color:var(--accent)}
.sl-arrow{color:var(--faint)}
.sl-sub,.sl-prov{font-size:18px;color:var(--faint);text-wrap:pretty}
.sl-det{font-family:var(--font-data);font-size:19px;color:var(--text-body)}
.sl-note{font-size:19px;color:var(--text-body);border-left:2px solid var(--border);
  padding-left:12px;margin-top:6px;max-width:90ch;text-wrap:pretty}
.sl-faint{color:var(--faint)}
.sl-tot{display:flex;flex-wrap:wrap;gap:0;row-gap:20px;padding-bottom:22px;
  border-bottom:1px solid var(--border);margin-bottom:34px}
.sl-tot>div{min-width:180px;max-width:34ch;flex:1 1 200px;padding:0 26px}
.sl-tot>div:first-child{padding-left:0}
.sl-tot>div + div{border-left:1px solid var(--border)}
.sl-k{font-size:18px;color:var(--faint)}
.sl-v{font-family:var(--font-data);font-variant-numeric:tabular-nums;font-size:33px;
  font-weight:600;margin:4px 0 2px}
.sl-n{font-size:18px;color:var(--faint);text-wrap:pretty}
