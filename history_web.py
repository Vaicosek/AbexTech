...
WHY THIS EXISTS
───────────────
A player bought 5,000,000 coins of Abex Tech stock by paying John in-game, off the
...
def counterparty(uid: Any) -> dict:
    ...
    if uid.startswith("treasury:"):
        return {"id": uid, "name": "Abex Tech treasury", "sub": uid, "linked": True}
...
_CSS = """
<style>
/* History — the ledger. Warm Feel: rules and space, no boxes, no caps. */
.h-strip{display:flex;flex-wrap:wrap;row-gap:20px;align-items:flex-start;
  padding-bottom:22px;border-bottom:1px solid var(--border);margin-bottom:34px}
.h-seg{background:none;padding:0 26px;flex:1 1 200px;min-width:0}
.h-seg:first-child{padding-left:0}
.h-seg + .h-seg{border-left:1px solid var(--border)}
.h-lab{font-size:18px;color:var(--faint)}
.h-val{font-family:var(--font-data);font-variant-numeric:tabular-nums;font-size:33px;
  font-weight:600;margin-top:4px}
.h-sub{font-size:18px;color:var(--faint);margin-top:4px;text-wrap:pretty}
/* Filters are words you can click, not a row of chips. */
.h-filters{display:flex;flex-wrap:wrap;gap:22px;margin:0 0 20px}
.h-chip{padding:0;border:none;background:none;font-size:21px;color:var(--text-body);
  font-family:var(--font-ui);text-decoration:none}
.h-chip:hover{color:var(--text)}
.h-chip.on{color:var(--accent);text-decoration:underline;text-underline-offset:3px}
.h-when{white-space:nowrap;font-family:var(--font-data);font-variant-numeric:tabular-nums;
  color:var(--faint)}
.h-kind{font-family:var(--font-ui);font-size:18px;color:var(--faint);white-space:nowrap}
.h-kind.otc{color:var(--red)}
.h-head{font-size:21px;color:var(--text)}
.h-det{font-size:19px;color:var(--text-body);margin-top:2px;text-wrap:pretty}
.h-prov{font-size:18px;color:var(--faint);margin-top:2px;text-wrap:pretty}
.h-note{font-size:19px;color:var(--text-body);margin-top:4px;
  border-left:2px solid var(--border);padding-left:12px;white-space:pre-wrap;
  overflow-wrap:anywhere}
.h-unk{color:var(--red)}
.h-empty{padding:6px 0;color:var(--faint);font-size:21px}
.h-page{display:flex;align-items:center;justify-content:space-between;gap:12px;
  margin-top:20px;font-size:19px;color:var(--text-body)}
.h-drift{color:var(--red);font-size:18px}
.h-kv{display:grid;grid-template-columns:260px 1fr;gap:6px 22px;font-size:21px}
.h-kv dt{color:var(--text-body);font-size:21px;padding-top:2px}
.h-kv dd{margin:0;overflow-wrap:anywhere}
@media (max-width:820px){.h-kv{grid-template-columns:1fr;gap:2px 0}
  .h-kv dt{padding-top:8px}}
</style>
"""
