...
def _os_env_home_guild() -> int:
    """HOME_GUILD_ID as used by _config_home_ok(); surfaced so the startup check can
    verify that guild still exists (it pins where /config may be run)."""
    try:
        return int(os.getenv("HOME_GUILD_ID", "954487497411403806") or 0)
    except Exception:
        return 0
...
def build_order_embed(order: dict, items_data: dict) -> discord.Embed:
    """The order card workers claim from.

    The item is the subject, so it is the title; the order number is an id and
    lives in the kicker and the footer. Pay is stated per piece AND per barrel
    with the conversion spelled out — the 64-to-1 confusion between a piece and a
    stack is the most repeated bug in this economy, and an order card is exactly
    where it costs someone real coins.
    """
    ...
    remaining = max(0, requested - assigned)
    is_futures = str(order.get("source", "")) == "futures"
    oid = order.get("id", "?")

    market = (_get_market(order.get("market_id")) or {}).get(
        "name", order.get("market_id") or "")
    want = fmt_qty(order, requested, prefer_original_amount=True)
    left = fmt_qty(order, remaining)
    desc = f"{want}{f' for {market}' if market else ''}. "
    desc += f"{left} still open." if remaining else "Fully claimed."
    ...
    groups = []
    sell_loc = _market_sell_location(order.get("market_id"))
    if sell_loc:
        groups.append(("Deliver to", sell_loc))
    if is_futures:
        cust = order.get("customer_id")
        groups.append(("Placed for", f"<@{cust}>" if cust else "a customer"))

    claims = order.get("claims") or []
    if claims:
        groups.append(("Claimed so far", ab.rows(
            [(c.get("user_tag", "unknown"), fmt_qty(order, int(c.get("qty", 0) or 0)))
             for c in claims[:10]])))
    # No claims yet renders nothing at all. A field holding an em-dash is a
    # decorated empty state.

    groups.append(("Pay", ab.rows([
        ("All of it", f"about {total_payout} coins"),
        ("Per piece", ab.coins(price_piece, 2)),
        ("Per barrel", f"{ab.coins(price_barrel, 2)} for {pieces_per_barrel} pieces"),
    ], strong="All of it")))

    return ab.embed(
        title=str(order.get("item", "") or f"Order {oid}"),
        kicker=f"Futures order {oid}" if is_futures else f"Order {oid}",
        desc=desc,
        groups=groups,
        foot=f"Order {oid} · {str(order.get('status', 'open')).lower()}",
    )
...
def deduct_coins(uid: int, amount: int, *, reduce_principal: bool = True, reason: str = "") -> tuple[int, int]:
    ...
    try:
        ...
        # SHORTFALL DETECTION (observability only — behaviour unchanged).
        # `adjust_balance` writes `coins = MAX(0, coins - ?)`, so debiting 1,000 from a
        # 300-coin wallet removes 300 and returns normally. This function returns only
        # (coins, principal), so the caller has NO WAY to notice it under-charged: a shop
        # sale, a bid or a fee can believe it took the full amount when it did not.
        # Nothing is changed here on purpose — the clamp may be load-bearing for some
        # caller — but it is now LOUD and auditable, so we can see whether it ever
        # actually fires before deciding to make it fail instead.
        try:
            short = amt - abs(int(applied))
            if short > 0:
                log.error("[deduct_coins] UNDER-CHARGED uid=%s asked=%s took=%s short=%s "
                          "reason=%r — the caller believes it charged the full amount.",
                          uid, amt, abs(int(applied)), short, reason)
                _db.record_coin_ledger(str(uid), 0, coins,
                                       f"SHORTFALL: asked {amt}, took {abs(int(applied))} "
                                       f"({reason or 'unlabelled'})")
        except Exception:
            log.debug("[deduct_coins] shortfall check failed", exc_info=True)
        return coins, principal
    except sqlite3.IntegrityError:
...
async def _process_csn_attachment(attachment: discord.Attachment, report_channel, source_channel_id=None,
    ...
    # The market is NOT repeated as a field: the footer below already names it, and a
    # card that says "GreyHames" twice reads as a template with the slots filled in.
    ...
    # The footer names scope — which market, and which upload it was built from. The
    # "auto-report from the CSN mod" half of the old footer moved to the kicker, where
    # the house shape puts the thing that produced the card. Nothing parses this text.
    footer = f"{market_name} · source {filename}" if market_name else f"Source {filename}"
...
async def on_ready():
    ...
    # Boot-time config check — read-only. Prints which configured ids actually resolve,
    # so a channel left pointing at a deleted server is visible in the Wisp console
    # instead of failing silently inside a fire-and-forget post.
    try:
        import config_check as _cfgchk
        await _cfgchk.report(
            bot, title="Restocker",
            guilds={"ADMIN_GUILD_ID": ADMIN_GUILD_ID,
                    "FUNDS_REPORT_GUILD_ID": FUNDS_REPORT_GUILD_ID,
                    "HOME_GUILD_ID": _os_env_home_guild()},
            channels={"WORKER_CHANNEL_ID": WORKER_CHANNEL_ID,
                      "WELCOME_CHANNEL_ID": WELCOME_CHANNEL_ID,
                      "TICKETS_CATEGORY_ID": TICKETS_CATEGORY_ID,
                      "FUNDS_REPORT_CHANNEL_ID": FUNDS_REPORT_CHANNEL_ID,
                      "DIVIDEND_REPORTS_CHANNEL_ID": DIVIDEND_REPORTS_CHANNEL_ID,
                      "INVESTOR_CHAT_CHANNEL_ID": INVESTOR_CHAT_CHANNEL_ID,
                      "FUTURES_CHANNEL_ID": FUTURES_CHANNEL_ID,
                      "NETWORK_FORUM_CHANNEL_ID": NETWORK_FORUM_CHANNEL_ID,
                      "LOAN_PROPOSALS/announce": _env_int("ANNOUNCE_CHANNEL_ID",
                                                          1500543188018921512)},
            notes=["ADMIN_GUILD_ID unset = every command syncs GLOBALLY"
                   if not ADMIN_GUILD_ID else "admin split active"])
    except Exception as _cce:
        print(f"⚠️ config check skipped: {_cce}")
...
def _build_csn_layout(embed: discord.Embed, footer: str, report_url: str,
    ...
    Presentation only: the embed carries every figure, this re-lays it out. The band
    (the embed's inline fields) collapses onto one line rather than becoming three
    headings, and the field the link button stands in for is skipped BY NAME — "Full
    report" today, and the old zero-width name kept alongside it so a card built by an
    older embed builder still renders without a duplicate link.
    ...
    try:
        ...
        accent = embed.color.value if embed.color else ab.ACCENT
        box = discord.ui.Container(accent_color=accent)

        kicker = ""
        try:
            kicker = (embed.author.name or "").strip() if embed.author else ""
        except Exception:
            kicker = ""
        head = f"-# {kicker}\n" if kicker else ""
        ...
        for f in embed.fields:
            ...
            # "Full report" is the link field the button below replaces; "​" is the
            # zero-width name the previous builder used for the same field.
            if not name or name in ("Full report", "​"):
                continue
            if getattr(f, "inline", False):
                band.append(f"{name} **{f.value}**")
            else:
                blocks.append(f"**{name}**\n{f.value}")
        if band:
            box.add_item(discord.ui.TextDisplay(" · ".join(band)))
        if band and blocks:
            box.add_item(discord.ui.Separator())
        for b in blocks:
            box.add_item(discord.ui.TextDisplay(b))
        ...
        row.add_item(discord.ui.Button(label="Open the full report",
...
#: Report vocabulary that arrives Title Cased from older captions. Sentence case is
#: the house rule, but a blanket lower() would eat "CSN", "GreyHames" and month names,
#: so only these exact words are lowered, and never the first word of the caption.
_REPORT_CAPTION_WORDS = frozenset({
    "Monthly", "Sales", "Report", "Reports", "Summary", "Profit", "Income", "Spent",
    "Market", "Markets", "Period", "Earners", "Needed", "Stock", "Items", "Item",
})


def _sanitise_report_label(text) -> str:
    """Tidy a caption built elsewhere into house style. Display only — no figure here.

    Titles and extra-field names reach the report builders already carrying emoji and
    Title Case from older call sites, and this is the only place that can fix them
    without editing the code that owns the figures. Pictographs are dropped, an em dash
    separator becomes the middle dot the rest of the house uses, and report vocabulary
    is lowered to sentence case. Nothing parses these strings — the persistent surfaces
    in this file recover their subject from a footer id or an embed url, never from a
    report caption — but the words themselves are left otherwise intact.
    """
    keep = "—–·’‘“”"
    out = "".join(ch for ch in str(text or "") if ord(ch) < 0x2000 or ch in keep)
    out = out.replace(" — ", " · ").replace(" – ", " · ")
    parts = out.split()
    for i in range(1, len(parts)):
        if parts[i] in _REPORT_CAPTION_WORDS:
...
def _build_csn_compact_embed(title, items, income, spent, market_id, month_key,
    ...
    House shape (`abex_embed`): kicker, title, one sentence, a band of exactly three
    figures, then labelled label/value groups, then a footer naming scope. This is a
    presentation rewrite only — every figure below is the same figure the old card
    printed, from the same source, in the same units. What changed is that the
    monospace mini-tables became rows, the emoji went, prices and counts say their
    unit, and the colour bar means one thing (accent for the filing, red for a month
    that lost money) instead of tracking the sign of the net.
    """
    import abex_embed as ab
    items = _apply_brew_aliases(items)
    net = float(income) - float(spent)
    sold_units = sum(int(v.get("sold_qty", 0) or 0) for v in items.values())
    report_url = f"https://dashboard.vaicosmarket.com/report/{market_id}/{month_key}"
    ...
    try:
        ...
        if prior:
            ...
            if pnet:
                ...
                plabel = months[prior[-1]].get("label", prior[-1])
                mom = f" {'Up' if d >= 0 else 'Down'} {abs(d):.0f}% on {plabel}."
    except Exception:
        pass

    _n_items = len(items)
    # A month with nothing in it says nothing. "0 pieces sold across 0 items" is a
    # placeholder wearing a sentence.
    desc = (f"{sold_units:,} piece{'' if sold_units == 1 else 's'} sold across "
            f"{_n_items:,} item{'' if _n_items == 1 else 's'}.{mom}"
            if _n_items else mom.strip())

    # int() BEFORE formatting. `:,.0f` rounds and the old card truncated, so a
    # restyle was quietly moving filed figures by a coin — on the one card an owner
    # files and an investor reads. `signed()` on a net in (-0.5, 0) also printed
    # "-0c"; truncation makes it "0c", which is what the old card said.
    band = [("Money in", ab.coins(int(income))),
            ("Money out", ab.coins(int(spent))),
            ("Net for the month", ab.signed(int(net)))]

    # Top earners — the part people missed from the old big card, as label/value rows
    # instead of a padded code block (monospace wraps badly at 380px on a phone).
    top = [kv for kv in sorted(items.items(), key=lambda kv: -float(kv[1].get("net_coins", 0) or 0))
           if float(kv[1].get("net_coins", 0) or 0) > 0][:5]
    top_rows = ""
    if top:
        pairs = [(f"{_pretty_item_name(n)} · {int(v.get('sold_qty', 0) or 0):,} sold",
                  ab.coins(float(v.get("net_coins", 0) or 0)))
                 for n, v in top]
        top_rows = ab.rows(pairs, strong=pairs[0][0])

    # Biggest buys (restock spend) — the expense side at a glance. The figure stays
    # negative, exactly as stored: it is money out.
    buys = [kv for kv in sorted(items.items(), key=lambda kv: float(kv[1].get("net_coins", 0) or 0))
            if float(kv[1].get("net_coins", 0) or 0) < 0][:3]
    buy_rows = ""
    if buys:
        buy_rows = ab.rows([(_pretty_item_name(n), ab.coins(float(v.get("net_coins", 0) or 0)))
                            for n, v in buys])
    ...
    # mod build), so the card carries exactly what it always did for them.
    # The day label stays the ledger's own calendar date rather than a Discord timestamp:
    # a trading day is a fact of the books, not an instant, and shifting it into each
    # reader's timezone would show some of them the wrong day.
    day_rows = ""
    try:
        ...
        # The field is headed "last five with activity", so a day with no sales does
        # not belong in it. GreyHames' August card listed two rows reading
        # "0c · 0 pieces · 1 buyers" under that heading.
        day_pairs = [(str(_d["day"]),
                      f"{ab.coins(int(_d.get('income') or 0))} · "
                      f"{int(_d.get('units') or 0):,} pieces · "
                      f"{int(_d.get('customers') or 0):,} "
                      f"buyer{'' if int(_d.get('customers') or 0) == 1 else 's'}")
                     for _d in _days
                     if int(_d.get('units') or 0) or int(_d.get('income') or 0)][:5]
        if day_pairs:
            day_rows = ab.rows(day_pairs)
    ...
    try:
        ...
        if low:
            low.sort()
            low_label = (f"Low stock, {len(low):,} item{'' if len(low) == 1 else 's'} "
                         f"at or below 20 per cent")
            low_rows = ab.rows([(f"{_pretty_item_name(i)} · {p * 100:.0f}% full",
                                 f"{n:,} pieces short") for p, i, n in low[:5]])
    except Exception:
        pass

    embed = ab.embed(
        kicker="Auto-posted from the CSN mod",
        title=_sanitise_report_label(title),
        url=report_url,
        desc=desc,
        band=band,
        groups=[("Top earners", top_rows),
                ("Biggest buys", buy_rows),
                ("Sales by day, last five with activity", day_rows),
                (low_label, low_rows),
                ("Full report", f"[Open the full report]({report_url})")],
        # One meaning per bar: the filing itself is official, a month that lost money
        # is a loss. Never green — green is money the reader actually received.
        colour=ab.LOSS if net < 0 else ab.ACCENT,
    )
    # Extras (period, market) are scope, not figures: full width, so the band above
    # stays exactly three tiles and never wraps to a second row on a phone.
    for name, value, _inline in (extra_fields or []):
        embed.add_field(name=_sanitise_report_label(name), value=value, inline=False)
    return embed
...
def _build_csn_embed(
    ...
    """The long-form CSN report card, in the house shape (`abex_embed`).

    Presentation only. Every figure, and the restock rule that produces it, is
    unchanged from the version this replaced; what changed is that the band is three
    parallel figures rather than five inline fields plus a blank spacer, the medals and
    emoji are gone, counts say pieces and barrels, and the bar means one thing.
    """
    import abex_embed as ab
    items = _apply_brew_aliases(items)
    net = income - spent
    ...
    band = [("Money in", ab.coins(income)),
            ("Money out", ab.coins(spent)),
            ("Net for the month", ab.signed(net))]

    top = sorted(items.items(), key=lambda x: x[1]["net_coins"], reverse=True)[:10]
    top_rows = ""
    if top:
        pairs = [(f"{_pretty_item_name(item)} · {int(v['sold_qty']):,} sold",
                  ab.coins(float(v["net_coins"])))
                 for item, v in top]
        top_rows = ab.rows(pairs, strong=pairs[0][0])
    ...
    restock_rows = ""
    if restock:
        restock_rows = ab.rows([(_pretty_item_name(item),
                                 f"{b:,} barrel{'' if b == 1 else 's'}")
                                for item, b in restock])

    embed = ab.embed(
        kicker="Auto-posted from the CSN mod",
        title=_sanitise_report_label(title),
        desc=(f"{total_units:,} piece{'' if total_units == 1 else 's'} sold across "
              f"{len(items):,} item{'' if len(items) == 1 else 's'}."),
        band=band,
        groups=[("Top earners", top_rows),
                ("Restock needed", restock_rows)],
        foot=_sanitise_report_label(source),
        # One meaning per bar: the filing is official, a month that lost money is a
        # loss. Green would claim the reader received this money; they did not.
        colour=ab.LOSS if net < 0 else ab.ACCENT,
    )

    if extra_fields:
        for fname, fvalue, finline in extra_fields:
            embed.add_field(name=_sanitise_report_label(fname), value=fvalue, inline=False)
        ...
        if mat and today >= mat[:10]:
            ...
            if tre >= due:
                ...
                _queue_dividend_post({"type": "bond_event", "market_id": mid,
                    ...
                    "lines": [f"Principal {int(due):,}c returned to {len(holders)} holder(s) at maturity."]})
            else:
                ...
                _queue_dividend_post({"type": "bond_event", "bad": True, "market_id": mid,
                    "title": f"Bond default · {label}",
                    "lines": [f"Treasury `{int(tre):,}c` can't cover principal `{int(due):,}c`.",
                              f"Bondholders hold FIRST CLAIM on the market's items — "
                              f"collateral `{int(col):,}c` on record."]})
            continue
            ...
            if tre >= coupon:
                ...
                if paid_now > 0:
                    _queue_dividend_post({"type": "bond_event", "market_id": mid,
                        ...
                        "lines": [f"`{int(coupon):,}c` ({float(b['coupon_pct']):g}%/mo on "
                ...
                _queue_dividend_post({"type": "bond_event", "bad": True, "market_id": mid,
                    "title": f"Missed bond coupon · {label} · {cur_month}",
                    "lines": [f"{mid} treasury `{int(tre):,}c` < coupon `{int(coupon):,}c` "
            ...
            if face > 0 and pct < BOND_MIN_ITEM_COVER and \
                ...
                _queue_dividend_post({"type": "bond_event", "bad": True, "market_id": _mid,
                    ...
                    "lines": [f"Item coverage fell to **{pct:.1f}%** "
                              f"(rule ≥{BOND_MIN_ITEM_COVER:g}%): items `{int(col):,}c` "
                              f"vs bond face `{int(face):,}c`.",
    ...
    for mid in (_db.get_public_markets() or {}):
        ...
        try:
            ...
            lines.append(f"Quality **{float(q.get('score') or 0)*100:.0f}/100** — "
                         ...
                         f"orders `{int(q.get('order_value_30d') or 0):,}c`/30d · "
        ...
        _queue_dividend_post({"type": "bond_event", "bad": not up, "market_id": mid,
            "title": f"Rating {'upgrade' if up else 'downgrade'} · "
                     f"{mid}: {prev} → {grade}",
            "lines": lines})
    ...
    for mid, lst in (_db.get_public_markets() or {}).items():
        try:
            ...
            try:
                ...
                grade = "?"
            lines = [f"Share {price:,.2f}c · cap {int(price * so):,}c · "
            ...
            if closed:
                ...
                lines.append(f"Net {last}: {months[last]:,.0f}c{delta}")
            lines.append(f"Backing {q['backed_pct']:.0f}% · traffic {q['visitors_month']:,}/mo · "
                         f"orders {q['order_value_30d']:,}c/30d · {q['history_months']} mo of reports")
            try:
                import json as _json
                cov = _json.loads(_db.get_config(f"bond_coverage:{mid}") or "null")
                if cov and cov.get("face"):
                    lines.append(f"Bonds outstanding {int(cov['face']):,}c · item coverage "
...
def _etf_info_embed():
    ...
    top = sorted(cons, key=lambda c: c.get("qmcap", c["mcap"]), reverse=True)[:15]
    holdings = ab.rows([
        ((_get_market(c["mid"]) or {}).get("name", c["mid"]),
         f'{100.0 * c.get("qmcap", c["mcap"]) / total_mcap:,.1f}% target, '
         f'{float(nav["holdings"].get(c["mid"], 0)):,.0f} shares held')
        for c in top])
    return ab.embed(
        title="Abex index fund",
        desc=f'{ab.coins(nav["nav"], 2)} a unit, {nav["units"]:,.2f} units outstanding.',
        band=[("Assets", ab.coins(nav["assets"])),
              ("Cash", ab.coins(nav["cash"])),
              ("Total", ab.coins(nav["total"]))],
        groups=[("Target weights, by quality-weighted market cap", holdings)],
        foot="Invest and redeem on the site.",
    )
...
def _build_stock_panel_embed(market_id: str) -> discord.Embed:
    ...
    House shape (`abex_embed`): the market's real name, linked to its page on the
    site, one sentence, three parallel figures, then the listing facts. The bar is
    the accent in every state — a panel is an official statement, and a colour
    that flips green and red per tick is decoration, not meaning.
    """
    import Restocker_db as _db
    import abex_embed as ab
    market = _get_market(market_id) or {}
    name = market.get("name", market_id)
    url = f"{DASHBOARD_URL}/shares/{market_id}"
    listing = _db.get_market_shares(market_id)
    if not listing or not listing.get("active"):
        return ab.embed(title=name, url=url, colour=ab.NEUTRAL,
                        desc="This market is not listed, so there are no shares to trade.")
    ...
    if change > 0:
        moved = f"up {ab.pct(pct)} since the last trade"
    elif change < 0:
        moved = f"down {ab.pct(abs(pct))} since the last trade"
    else:
        moved = "unchanged since the last trade"

    groups = [("Listing", ab.rows([
        ("Shares outstanding", f"{shares_out:,.0f}"),
        ("Last priced", str(listing.get("last_priced_month") or "not yet priced")),
    ]))]
    if spark:
        groups.append(("Last 24 trades", spark))

    return ab.embed(
        title=name,
        url=url,
        desc=f"{ab.coins(price, 2)} a share, {moved}.",
        band=[("Share price", ab.coins(price, 2)),
              ("Market cap", ab.coins(mcap)),
              ("P/E", ab.multiple(listing["pe_multiplier"], 1))],
        groups=groups,
        foot="The price moves with each trade. Your confirmation is private.",
    )


def _panel_market_from_message(interaction: discord.Interaction) -> Optional[str]:
    """Recover the market_id from a panel message, so the view keeps working
    after a bot restart even when its in-memory market_id is gone.

    The id now travels in the title's link (`/shares/<mid>`) rather than in
    backticks beside the name — an id printed next to the thing it identifies is
    for the bot's benefit, not the reader's. Panels posted before that change are
    still in channels and still have to work, so the old backtick parse stays as
    the fallback. Drop it only once no pre-existing panel can still be clicked.
    """
    try:
        ...
        return None
    try:
        m = re.search(r"/shares/([^/?#]+)", embed.url or "")
        if m:
            return m.group(1)
    except Exception:
        pass
    try:
        toks = re.findall(r"`([^`]+)`", embed.title or "")
...
def _build_market_dashboard_embed() -> discord.Embed:
    ...
    One line per market, in the feed shape John accepted: subject, then figures.
    No arrows, no coin emoji, and no ticker stuck to the name — the ticker is an
    id and this is a list people read.
    """
    import Restocker_db as _db
    import abex_embed as ab
    public = _db.get_public_markets()
    if not public:
        # Empty is empty: the one thing that would change it, and nothing else.
        return ab.embed(title="Market exchange", colour=ab.NEUTRAL,
                        desc="No markets are listed yet. An owner lists one from /my market.")
    ...
    for mid, lst in ordered:
        ...
        prev = float(hist[1]["price"]) if len(hist) > 1 else price
        chg = price - prev
        pc = (chg / prev * 100.0) if prev else 0.0
        moved = ab.pct(pc) if chg else "unchanged"
        lines.append(ab.line(name, f"{ab.coins(price, 2)} a share", moved,
                             f"{ab.coins(price * shares)} market cap"))

    e = ab.embed(title="Market exchange",
                 desc="\n".join(lines),
                 foot=f"{len(ordered)} listed markets, largest first. Updates every 5 minutes.")
...
async def _main():
    ...
    # /announce — write an announcement, preview it, then post it to the announcements
    # channel. NOT in PUBLIC_COMMAND_NAMES on purpose: it is company plumbing, so the
    # sync split keeps it admin-guild-only and it costs ZERO public command surface.
    try:
        import announce as _announce
        _announce.DEFAULT_CHANNEL_ID = _env_int("ANNOUNCE_CHANNEL_ID", 1500543188018921512)
        _announce.FOOTER = os.getenv("ANNOUNCE_FOOTER", "").strip()
        _announce.install_commands(bot.tree)
    except Exception as e:
        log.error("/announce install failed: %s", e)
    # ── Login pre-flight ──────────────────────────────────────────────────────
