...
def _balance_payload(user_id: str) -> dict:
    """`lp` used to read `balances.lp`, which NOTHING writes — real loyalty lives in the
    `loyalty` table. So every consumer trusting this field saw 0 forever. It now reports
    the true balance, plus the tier, so a service can price a loyalty benefit without a
    second round trip. `loyalty_points` is the name to prefer; `lp` stays as an alias so
    no existing caller breaks."""
    b = _db().get_balance(str(user_id))
    points, tier = 0.0, None
    try:
        rec = _db().get_loyalty(str(user_id)) or {}
        points = float(rec.get("points") or 0)
        import Restocker_main as _m
        tier = dict(_m._loyalty_tier(points))
    except Exception:
        log.debug("loyalty lookup failed for %s", user_id, exc_info=True)
    return {
        "user_id": str(user_id),
