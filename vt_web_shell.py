"""
vt_web_shell.py — shared chrome, identity and form-idempotency for the Abex Tech
...
def refuse_if_impersonating(request) -> Optional[Any]:
    ...
    return json_err(
        "view_as_read_only",
        f"You are viewing Abex Tech as {va.get('target_name') or va['target_id']}. "
...
async function renderStrip(){
  ...
  el('sSaveSub').textContent = j.savings === null
    ? (j.bank_error || 'Abex Bank unreachable')
    : (j.savings_apr != null ? j.savings_apr + '% APR' : 'Abex Bank');
  ...
  if(note) note.textContent = j.net === null
    ? 'withheld — Abex Bank is unreachable, so savings and loan debt are unknown'
...
<div class="strip-wrap" id="stripWrap">
  <div class="strip">
    ...
    <div class="seg">
      <div class="lab">Savings · Abex Bank</div>
    ...
    <div class="header-right"><span class="user-tag" id="authTag"></span></div>
  </div>
  <div class="drawer">
...
#: Pages that ask for no money strip still need the header row the strip
#: normally provides — it carries `#authTag`, which the base script fills on
#: every page. Without this a `strip=False` page rendered with no header at
#: all and no signed-in name.
_BARE_HEAD_HTML = """
<div class="strip-wrap" id="stripWrap">
  <div class="strip">
    <div class="spacer"></div>
...
def page_scripts(page_js: str = "") -> str:
    """The confirm modal and the page's scripts, as one block for the shell's tail.

    Pulled out of `page()` so a section that renders through `abex_shell` directly
    — `abex_web` does — gets the same modal and the same base script rather than a
    second, drifting copy of this assembly.
    """
    return (_MODAL_HTML + "<script>\n" + _ICONS_JS
            + "\ndocument.querySelectorAll('.nav-ic').forEach(e => "
...
def page(title: str, nav_key: str, body: str, page_js: str = "", strip: bool = True) -> Any:
    ...
    The chrome comes from `abex_shell` — the same shell the Abex screens use, so
    there is one sidebar, one nav and one theme in the product rather than three.
    What this function still owns is what is particular to a transactional page:
    the live money strip in the header, the confirm modal, and the page script.

    Same templating convention as the rest of `Restocker_web`: a Python string,
    no Jinja, no template directory, no static files.
    """
    import abex_shell
    ...
    doc = abex_shell.render(
        ...
        extra_css=_LEGACY_CSS,
        header=_view_as_banner_html(ctx) + head,
        tail=tail,
        available=_MOUNTED,
        paths=_NAV_PATHS,
    )
...
#: The class vocabulary these pages still write. It loads after `abex_theme`'s
#: stylesheet, so a page can be ported to the theme's own classes one at a time
#: instead of all at once. Everything about the chrome — the ground, the type,
#: the sidebar, the nav — comes from the theme; nothing here redefines it.
_LEGACY_CSS = r"""
/* These pages were written against the previous token names. Rather than
   rewrite 1,200 declarations, the old names are aliased onto the theme's — one
   place, and it disappears as pages move to the theme's own classes. A missing
   alias is not a subtle bug: every colour drops to the browser default, which
   is how "84,230c" briefly stopped being green. */
:root{
  --bg:var(--ground);--surface:transparent;--panel2:var(--raised);
...
.seg .lab{font-size:18px;color:var(--faint);margin-bottom:2px;display:flex;
  align-items:center;gap:6px}
.seg .val{font-family:var(--font-data);font-variant-numeric:tabular-nums;
  font-size:21px;font-weight:600;line-height:1.2}
.seg .sub{font-size:14px;color:var(--faint);margin-top:1px}
.v-avail{color:var(--money-available)}
.v-held{color:var(--money-held)}
.v-save{color:var(--green)}
.v-net{color:var(--green)}
.coin{font-family:var(--font-data);font-size:.62em;color:var(--faint);margin-left:3px}
.seg.held-seg{cursor:pointer;text-align:left}
.seg.held-seg:hover .lab{color:var(--text)}
.seg.held-seg .lab{color:var(--faint)}
.chev{display:inline-flex;transition:transform .18s ease;color:var(--faint)}
...
.net-note{font-size:14px;color:var(--faint)}
.header-right{margin-left:30px;flex:0 0 auto;align-self:center;white-space:nowrap}
.user-tag{display:flex;align-items:center;gap:9px;color:var(--text-body);font-size:19px}
.user-mark{display:none}
.auth-name{color:var(--text-body);font-weight:400}
.svc-note{font-size:18px;color:var(--faint)}
.drawer{max-height:0;overflow:hidden;transition:max-height .22s ease;
  border-top:1px solid transparent}
.strip-wrap.open .drawer{max-height:520px;border-top-color:var(--border)}
.drawer-inner{padding:14px 60px 18px}
.drawer-head{font-size:18px;color:var(--faint);margin-bottom:9px;display:flex;
  align-items:center;gap:10px}
.drawer-head::after{content:"";flex:1;height:1px;background:var(--border)}
.hold-row{display:flex;align-items:center;gap:12px;width:100%;text-align:left;
  padding:9px 0;border:none;border-bottom:1px solid var(--border);background:none;
  cursor:pointer}
.hold-row:hover{background:var(--panel2)}
.hold-dot{width:8px;height:8px;border-radius:50%;flex:0 0 auto;display:inline-block}
.hold-t{font-size:21px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}
.hold-main{flex:1;min-width:0}
.hold-s{font-size:18px;color:var(--faint);display:block}
.hold-amt{font-size:21px;font-weight:600;color:var(--money-held);white-space:nowrap}
.hold-go{color:var(--faint);display:flex}
.drawer-foot{display:flex;justify-content:space-between;align-items:center;gap:16px;
  padding-top:9px;margin-top:6px;border-top:1px solid var(--border);font-size:18px;
  color:var(--faint)}
.holdgap{font-size:18px;color:var(--faint);padding:9px 0 0;margin-top:6px;
...
.tile-h{font-size:28px;font-weight:400;color:var(--text);margin-bottom:10px;
  display:flex;align-items:center;justify-content:space-between;gap:10px}
.eyebrow{font-size:18px;color:var(--faint);margin-bottom:4px}
.q{font-size:21px;font-weight:700;color:var(--text)}
.t-title{font-size:21px;font-weight:700}
.bento{display:grid;grid-template-columns:repeat(12,1fr);gap:34px;margin-bottom:34px}
.tile{background:none;border:none;border-top:1px solid var(--border);padding:20px 0 0;
  position:relative}
...
.s7, .s6, .s5, .s4, .s3{grid-column:span 12}
}
.hubcard{border-top:2px solid var(--accent-c);cursor:pointer;display:flex;
  flex-direction:column;gap:14px}
.hubcard:hover{background:var(--panel2)}
.hubcard .hc-name{display:flex;align-items:center;gap:8px;font-size:28px;font-weight:400}
.hubcard .desc{color:var(--text-body);font-size:18px;margin-top:4px;text-wrap:pretty}
...
.fig .fv{font-size:33px;font-weight:600}
.fig .fl{font-size:18px;color:var(--faint);margin-top:2px}
.hubcard .go{font-size:19px;color:var(--accent);display:flex;align-items:center;gap:6px}
.badge-dot{width:8px;height:8px;background:var(--accent-c);display:inline-block;
  border-radius:50%;}
.kgrid{display:grid;grid-template-columns:1fr 1fr;gap:0}
.kpi{background:none;border:none;border-right:1px solid var(--border);padding:0 26px}
.kgrid .kpi:last-child{border-right:none}
...
.outcome{display:grid;grid-template-columns:1fr auto;gap:4px 14px;align-items:center;
  padding:10px 0;border:none;border-bottom:1px solid var(--border);background:none;
  margin-bottom:0}
.outcome.mine{background:rgba(201,179,122,.07)}
.o-name{font-size:21px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.o-right{text-align:right;font-size:21px;white-space:nowrap}
.o-odds{font-weight:700}
.o-barwrap{grid-column:1/-1;display:flex;align-items:center;gap:10px;margin-top:4px}
.o-barwrap .bar{flex:1}
.o-pct{font-size:18px;color:var(--faint);width:46px;text-align:right;
  font-family:var(--font-data);font-variant-numeric:tabular-nums}
.lot{border:none;border-top:2px solid var(--lc);background:none;display:flex;
  flex-direction:column}
.lot-body{padding:14px 0;display:flex;flex-direction:column;gap:10px;flex:1}
.lot-id{font-family:var(--font-data);font-size:18px;color:var(--faint)}
.lot-t{font-size:28px;font-weight:400;margin-top:5px}
.lot-m{font-size:18px;color:var(--faint)}
.lot-bid{font-size:33px;font-weight:600}
.chart-legend{display:flex;gap:16px;font-size:18px;color:var(--faint);align-items:center}
.lg{display:flex;align-items:center;gap:6px}
.lg i{width:12px;height:2px;display:block}
.scrim{position:fixed;inset:0;background:rgba(12,13,15,.86);z-index:200;display:none;
  align-items:center;justify-content:center;padding:20px}
.scrim.on{display:flex}
.modal{width:100%;max-width:620px;background:var(--bg);border:1px solid var(--border-strong);
  max-height:92vh;display:flex;flex-direction:column}
.modal-head{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;
  justify-content:space-between;align-items:flex-start;gap:12px}
.modal-head h3{margin:0;font-size:28px;font-weight:400}
.modal-head .mh-sub{font-size:18px;color:var(--text-body);margin-top:3px;text-wrap:pretty}
.modal-body{padding:20px;overflow-y:auto}
.modal-foot{padding:14px 20px;border-top:1px solid var(--border);display:flex;gap:10px;
  justify-content:flex-end;flex-wrap:wrap}
.x{color:var(--faint);display:flex;padding:2px}
.x:hover{color:var(--text)}
.amount-wrap{position:relative;margin-bottom:10px}
.amount{width:100%;padding:13px 14px 13px 44px;font-size:33px;font-weight:600;
  background:none;border:1px solid var(--border);color:var(--text);outline:none;
  font-family:var(--font-data);font-variant-numeric:tabular-nums}
...
.kv{display:flex;justify-content:space-between;gap:16px;padding:5px 0;font-size:21px;
  border-bottom:none}
.kv .k{color:var(--text-body)}
.kv .v{font-weight:400;white-space:nowrap;text-align:right;font-family:var(--font-data);
  font-variant-numeric:tabular-nums}
.kv.total{border-top:1px solid var(--border);margin-top:6px;padding-top:9px;
  font-size:21px}
.kv.total .k, .kv.total .v{font-weight:700;color:var(--text)}
.notebox, .okbox{margin-top:14px;padding:0;font-size:18px;line-height:1.55;
  background:none;border:none;color:var(--text-body);text-wrap:pretty}
.notebox.blue{color:var(--text-body)}
.big-ok{display:none}
...
.bank-down .bd-h{font-size:21px;color:var(--red);margin-bottom:4px}
.bank-down .bd-b{font-size:19px;color:var(--text-body);text-wrap:pretty}
.bank-down .bd-b code{font-family:var(--font-data);font-size:18px;color:var(--faint)}
.holdnote{font-size:18px;color:var(--faint);margin-top:6px;text-wrap:pretty}
...
.seg.net-seg{align-items:flex-start;text-align:left}
.header-right{margin-left:0;width:100%;margin-top:8px}
.drawer-inner{padding:12px 20px 16px}
.hide-sm{display:none}
th.hide-sm, td.hide-sm{display:none}
.kgrid{grid-template-columns:1fr}
.kpi{border-right:none;padding:0}
.bento{gap:24px}
}
"""
...
#: this is what exists. Without it the nav offers pages that 404.
...
async def _handle_strip(request):
    ...
    try:
        ...
        bank_error = str(e) or "Abex Bank unreachable"
...
def _describe_hold(h: dict, uid: str) -> tuple:
    ...
    # which service reserved it — "Abex Bank — loan:collateral" is a row a player
