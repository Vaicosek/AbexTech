    ...
    try:
        ...
        with db.db() as conn:
            ...
            for m in conn.execute("SELECT market_id, shares_outstanding, share_price, "
                ...
                # greyhames carries the Abex Tech listing (stock_label:greyhames = "Abex Tech"),
                # and a page about what Abex Tech owes should say Abex Tech.
...
def _load_stock_data() -> dict:
    ...
    try:
        ...
        pass
    # A stock can carry a COMPANY label distinct from its host market's name (the Abex Tech
...
_TERMINAL_CSS = r"""
/* Abex Tech — Warm Feel. Ported from `Abex Tech Screens.dc.html` (skin "Warm
   Feel", its default) so the terminal pages read as the same product as the
   hub. Token names are unchanged; only the values, the type scale and the
   surfaces moved. Layout is untouched. */
:root{--bg:#1b1d20;--panel:transparent;--panel2:transparent;--row:transparent;
--hover:rgba(239,236,229,.05);--sel:rgba(201,179,122,.07);
--seam:#3b3e43;--line:#3b3e43;--line2:#4a4e54;--ink:#efece5;--ink2:#efece5;
--muted:#aaa59b;--faint:#6f6c66;
--up:#8fbf6a;--down:#d87a6a;--accent:#c9b37a;--amber:#d87a6a;
...
header.tshell nav a.on{color:var(--ink);border-bottom-color:var(--accent)}
header.tshell nav a:hover{color:var(--ink);text-decoration:none}
.rt{margin-left:auto;display:flex;align-items:center;gap:22px}
.rt .bp{text-align:right;line-height:1.3}
.rt .bp b{font-size:21px;font-family:var(--mono);font-variant-numeric:tabular-nums}
.rt .bp span{font-size:18px;color:var(--faint)}
/* No panel fills in this skin: a block is separated by a rule and by space. */
.panel{background:none;border:none;border-top:1px solid var(--line);margin-bottom:34px}
.ph{height:auto;display:flex;align-items:center;justify-content:space-between;
padding:18px 0 8px;background:none;border-bottom:none}
.ph .t{font-size:28px;letter-spacing:normal;text-transform:none;color:var(--ink);
font-weight:400}
.content{max-width:1640px;margin:0 auto;width:100%;padding:0 26px}
@media(min-width:1300px){.content{border-left:none;border-right:none}}

/* The shell chrome. The nav is eight-plus words on one line and was wrapping
   "My Market" onto two; the brand was set in two tones, which reads as a logo
...
/* Inventory, in Warm Feel. The layout was built for the dense dark skin: filled
   panels on 1px seams, chip rows, caps micro-labels. What replaces each of those
   here is a rule and some space. Nothing about what the page shows changed. */

.wrap{display:block}
/* A filter row is words you can click. The bordered pills were the loudest chip
   surface left in the product — two full rows of them above the table. */
.bar{display:flex;align-items:center;gap:22px;padding:14px 26px;background:none;
  border-bottom:1px solid var(--line);flex-wrap:wrap}
.chip{border:none;background:none;color:var(--muted);font-family:var(--sans);
  font-size:19px;padding:0;cursor:pointer;white-space:nowrap}
.chip.on{color:var(--accent);box-shadow:none;text-decoration:underline;
  text-underline-offset:3px}
.chip:hover{color:var(--ink)}
.search{margin-left:auto;background:none;border:1px solid var(--line);color:var(--ink);
  font-family:var(--sans);font-size:19px;padding:8px 12px;width:260px;outline:none}
.search:focus{border-color:var(--accent)}
.gen{border:1px solid var(--line);background:none;color:var(--muted);
  font-family:var(--sans);font-size:19px;letter-spacing:normal;text-transform:none;
  padding:7px 14px;cursor:pointer}
.gen:hover{background:none;border-color:var(--line2);color:var(--ink)}

/* The stat band: rule-separated, sentence case, no fill. */
.statrow{display:flex;flex-wrap:wrap;row-gap:20px;gap:0;background:none;
  padding:22px 26px;border-bottom:1px solid var(--line)}
.stat{background:none;padding:0 26px;flex:1 1 200px;min-width:0;border-bottom:none;
  border-right:1px solid var(--line)}
.stat:first-child{padding-left:0}
.stat:last-child{border-right:none}
.stat .k{font-size:18px;letter-spacing:normal;text-transform:none;color:var(--faint);
  font-weight:400}
.stat .v{font-family:var(--mono);font-size:33px;font-weight:600;margin-top:4px;
  font-variant-numeric:tabular-nums}

table.inv{width:100%;border-collapse:collapse;table-layout:fixed}
table.inv th{font-size:21px;letter-spacing:normal;text-transform:none;color:var(--faint);
...
table.inv th:nth-child(2){width:230px}
table.inv th:nth-child(3),table.inv th:nth-child(4){width:130px}
table.inv th:nth-child(5){width:200px}
table.inv td:first-child{overflow:hidden;text-overflow:ellipsis}
tr.zero td:first-child{color:var(--muted)}
/* The market tag beside an item name is an id. Muted, not accent-coloured: the
   accent means "you can click this". */
table.inv td .mkt{color:var(--faint);font-size:17px;margin-left:10px;
  font-family:var(--sans);text-transform:none;letter-spacing:normal;opacity:1}
tr.zero .pct{opacity:.5}
table.inv th.sorted{color:var(--ink)}
table.inv td{padding:0 14px;height:38px;border-bottom:1px solid var(--line);
...
.fillcell{display:flex;align-items:center;gap:10px;justify-content:flex-end}
.fillbar{width:120px;height:6px;background:var(--hover);border:none;position:relative}
.fillbar i{position:absolute;left:0;top:0;bottom:0;display:block}
.pct{font-family:var(--mono);font-size:19px;width:52px;text-align:right}
/* Empty is empty: one line, left where the content would have been. */
.empty{padding:14px 26px;text-align:left;color:var(--faint);font-size:21px}
.msg{font-size:18px;color:var(--muted);font-family:var(--sans)}
.per{color:var(--faint);font-size:17px;font-family:var(--sans)}
.catbar{overflow-x:auto}
table.inv tr.grp td{background:none;color:var(--ink);font-family:var(--sans);
  font-size:25px;letter-spacing:normal;text-transform:none;font-weight:400;height:52px;
  text-align:left;border-bottom:1px solid var(--line);border-top:1px solid var(--line)}
table.inv tr.grp:hover td{background:none}
table.inv tr.grp td .gcount{color:var(--faint);margin-left:10px;font-family:var(--mono)}

/* These pages scope their table rules (`table.t th`), so a bare `th` override
   loses on specificity and the caps micro-heads survive. Same for the status
   tag, the form labels and the buttons. */
table.t th,table.inv th,table.cart th{font-size:19px;letter-spacing:normal;
  text-transform:none;color:var(--faint);font-weight:400;background:none;
  padding:10px 14px}
table.t td,table.cart td{font-size:20px;height:38px;padding:0 14px;
  border-bottom:1px solid var(--line)}
table.cart td{height:auto;padding:4px 14px 4px 0}
.tag{display:inline-block;font-size:20px;font-weight:400;letter-spacing:normal;
  text-transform:none;background:none !important;border:none !important;padding:0 !important;
  color:var(--muted)}
.fld label{font-size:18px;letter-spacing:normal;text-transform:none;color:var(--faint)}
.fld input{background:none;border:1px solid var(--line);color:var(--ink);
  font-family:var(--sans);font-size:20px;padding:8px 12px}
.btn{border:1px solid var(--accent);background:var(--accent);color:var(--bg);
  font-family:var(--sans);font-size:19px;font-weight:700;letter-spacing:normal;
  text-transform:none;padding:8px 14px}
.btn:hover{background:#d8c48f;border-color:#d8c48f;color:var(--bg)}
.btn.go:hover{background:#d8c48f}
.place{background:none;padding:14px 26px}
.pbar{border:none;background:var(--hover);height:6px;width:130px}
</style></head><body>
__NAV__
<div class="content">
...
<th data-k="stock">In stock</th><th data-k="capacity">Capacity</th><th data-k="price">Price, c per piece</th>
...
/* --- Warm Feel corrections, shared by the terminal pages -----------------
   These pages were laid out for the dense dark skin: filled panels on 1px
   seams, chip rows, caps micro-labels, 9-12px type. The palette moved with
   _TERMINAL_CSS; this is the rest of it. Layout and behaviour are untouched. */

/* Filters are words you can click, not bordered pills. */
.chip{border:none;background:none;color:var(--muted);font-family:var(--sans);
  font-size:19px;padding:0;letter-spacing:normal;text-transform:none}
.chip.on{color:var(--accent);border:none;box-shadow:none;background:none;
  text-decoration:underline;text-underline-offset:3px}
.chip:hover{color:var(--ink)}
.chips,.chipbar,.bar{gap:22px}

/* Stat bands: rule-separated, sentence case, no fill. */
.statrow{display:flex;flex-wrap:wrap;gap:0;row-gap:20px;background:none;
  padding:22px 26px;border-bottom:1px solid var(--line)}
.statrow .stat{background:none;padding:0 26px;flex:1 1 200px;border-bottom:none;
  border-right:1px solid var(--line)}
.statrow .stat:first-child{padding-left:0}
.statrow .stat:last-child{border-right:none}
.stat .k,.stat .lab{font-size:18px;letter-spacing:normal;text-transform:none;
  color:var(--faint);font-weight:400}
.stat .v{font-family:var(--mono);font-size:33px;font-weight:600;margin-top:4px}

/* Tables: reading size, sentence-case heads, rules instead of fills. */
th{font-size:19px;letter-spacing:normal;text-transform:none;color:var(--faint);
  font-weight:400;background:none}
td{font-size:20px}
tr:hover td{background:var(--hover)}

/* A status is a fact in a column. */
.status,.pill,.tag,.badge{border:none;background:none;padding:0;font-size:20px;
  letter-spacing:normal;text-transform:none}
.ph .t,.panel-h,.hd{font-size:21px;letter-spacing:normal;text-transform:none;
  color:var(--ink)}
.note,.hint,.sub,.msg{font-size:18px;letter-spacing:normal;text-transform:none}
/* Empty is empty: one muted line where the content would be. */
.empty{padding:14px 26px;text-align:left;font-size:21px;color:var(--faint)}
...
const ICOLS=[['item','Item',null],['s','Sold',r=>r.s],['b','Bought',r=>r.b],
 ['inc','Income',r=>r.inc],['exp','Expense',r=>r.exp],['net','Netc',r=>r.net],
...
function render(){const m=MS[act]||{};const mo=m.months||[];
 ...
 document.getElementById('stats').innerHTML=[
  ['Lifetime net',fmt(life)+'c','style="color:'+(life>=0?css('--up'):css('--down'))+'"'],
  ['Months tracked',mo.length,''],
  ['Best month',best?(best.label+' · '+fmt(best.net)):'—',''],
  ['Last month net',last?fmt(last.net)+'c':'—',last?('style="color:'+(last.net>=0?css('--up'):css('--down'))+'"'):'']]
 ...
 el.onmousemove=e=>{const r=el.getBoundingClientRect();let i2=Math.round((e.clientX-r.left)/r.width*(v.length-1));
  ...
  tip.textContent=(mo[i2]?mo[i2].label+': ':'')+fmt(v[i2])+'c';};
...
function cartR(){const t=document.getElementById('cart');
 t.innerHTML=cart.map((c,i)=>{const px=(ITEMS[c.item]&&ITEMS[c.item].coin)||0;
  ...
  '<td class="mono muted">'+(px?('≈ '+fmt(px*c.qty)+'c'):'')+'</td>'+
...
<table class="t"><thead><tr><th style="width:40px">#</th><th>Team</th><th>Members</th><th>Orders</th><th>Orderc</th><th>Salesc</th><th>Futures</th><th>Totalc</th></tr></thead>
<tbody id="tb"></tbody></table>
...
<div class="cols">
  <div class="panel"><div class="ph"><span class="t">Set item price / stock</span></div><div class="pb">
    <div class="row">
      ...
      <div class="fld"><label>Pricec/unit</label><input id="sp" type="number" min="0" step="0.01" style="width:100px"></div>
  ...
  <div class="panel"><div class="ph"><span class="t">Log a restock</span></div><div class="pb">
    <div class="row">
      ...
      <div class="fld"><label>Costc (total)</label><input id="rc" type="number" min="0" style="width:110px"></div>
...
const RCOLS=[['item','Item',null],['sold','Sold',r=>r.sold],['bought','Bought',r=>r.bought],
 ['income','Income',r=>r.income],['expense','Expense',r=>r.expense],['net','Netc',r=>r.net],
...
function renderReport(){
 ...
 document.getElementById('rstats').innerHTML=[
  ['Income',fmt(inc)+'c','style="color:var(--up)"'],
  ['Spent',fmt(sp)+'c','style="color:var(--down)"'],
  ['Net profit',(net>=0?'+':'')+fmt(net)+'c','style="color:'+(net>=0?'var(--up)':'var(--down)')+'"'],
  ['Items',its.length,''],
...
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600&family=Space+Grotesk:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
/* Reskinned to match the Abex Tech design tokens (hub_web.py THEME_CSS): same
...
/* Abex Tech — Warm Feel. Same skin as the rest of the site: flat ground, no panel
   fills, serif, one gold accent. This page kept its own copy of the old token
   block, so it stayed the only screen in the product still rendering in the dark
   green terminal palette. Layout untouched; the palette, the type scale and the
   caps micro-labels moved. */
:root{--bg:#1b1d20;--panel:transparent;--panel2:transparent;--row:transparent;
--hover:rgba(239,236,229,.05);--sel:rgba(201,179,122,.07);
--seam:#3b3e43;--line:#3b3e43;--line2:#4a4e54;--ink:#efece5;--ink2:#efece5;
--muted:#aaa59b;--faint:#6f6c66;
...
/* --- Warm Feel corrections on top of the layout ------------------------- */
/* Caps micro-labels were the loudest tell on the rejected version, and this page
   still had them on every panel head, column head and tile. */
[class]{letter-spacing:normal}
.hd,.ph .t,.panel-h,th,.lab,.k,.sub,.tag,.small{text-transform:none !important;
  letter-spacing:normal !important}
th{font-size:19px;font-weight:400;color:var(--faint)}
td{font-size:20px}
...
.side{background:none;border:1px solid var(--line);color:var(--muted);
  font-size:20px;letter-spacing:normal;text-transform:none;font-weight:400}
.side.on{background:var(--accent);border-color:var(--accent);color:var(--bg);font-weight:700}
.side.sell.on{background:var(--accent);border-color:var(--accent);color:var(--bg)}
.big,.price{font-family:var(--mono);font-variant-numeric:tabular-nums}

/* The real button classes on this page. Buy and sell are two directions of one
   action; a green light and a red one make the pair read as go/stop. The accent
   marks whichever side you are on. */
.toggle{background:none;border:1px solid var(--line);gap:0}
.toggle button{font-weight:400;font-size:20px;letter-spacing:normal;
  text-transform:none;background:none;color:var(--muted);padding:10px}
.toggle button.buy.on,.toggle button.sell.on{background:var(--accent);color:var(--bg);
  font-weight:700}
/* Panel heads and field labels: sentence case, at reading size. */
.ph .t{font-size:21px;color:var(--ink)}
.lab,.hint,.sub{font-size:18px;color:var(--faint);text-transform:none;letter-spacing:normal}
.cmd{font-size:19px;border-color:var(--line);background:none}
/* A grade is text, not a pill — same rule as every other screen. */
.tag,.badge,.grade{background:none !important;border:none !important;padding:0 !important;
  font-weight:700;font-size:20px}
.lbl{text-transform:none;letter-spacing:normal;font-size:18px;color:var(--faint)}
</style></head>
<body>
<header>
  <div class="brand"><img class="m" src="/static/atech-logo.png" alt="" width="44" height="44">Abex Exchange</div>
  ...
  <aside class="col">
    <div class="panel pb">
      ...
      <div class="field"><span class="lbl">Amount (coins)</span>
        <div class="inp"><input id="amt" class="mono" value="100000"/><span class="u">c</span></div>
...
function drawMain(m){const el=document.getElementById('chart');const w=el.clientWidth||740,h=250,pad=12;
 ...
 el.onmousemove=e=>{const r=el.getBoundingClientRect();let i=Math.round((e.clientX-r.left)/r.width*(v.length-1));
  ...
  tip.style.opacity=1;tip.textContent=fmt(v[i])+'c';};
...
function render(){const m=MK.find(x=>x.mid===cur);if(!m)return;
 ...
 const S=[['Mkt cap',fmt(m.mcap)+'c'],['P/E',(+m.pe).toFixed(1)+'x'],
  ...
  ['Treasury',fmt(m.treasury)+'c'],['Holders',m.holders_count],
...
function renderList(){const tot=MK.reduce((a,m)=>a+m.mcap,0);
 document.getElementById('wCap').textContent=fmt(tot)+'c';
...
function renderPos(){const m=MK.find(x=>x.mid===cur);const el=document.getElementById('pos');
 ...
 el.innerHTML=h?('<div class="kv">Shares<b>'+fmt(h.shares)+'</b></div><div class="kv">Value<b>'+fmt(h.value)+'c</b></div>'+
  '<div class="kv" style="border-bottom:0">Cost basis<b>'+fmt(h.cost)+'c</b></div>')
 ...
 if(ME.holdings&&ME.holdings.length){const tot=ME.holdings.reduce((a,x)=>a+x.value,0);
  ...
  pf.innerHTML='<div class="kv" style="font-size:15px;color:var(--ink2)"><span></span><b>'+fmt(tot)+'c</b></div>'+
  '<div class="kv">Total P/L<b class="'+(pl>=0?'up':'down')+'">'+(pl>=0?'+':'')+fmt(pl)+'c</b></div>'+
...
function calc(){const m=MK.find(x=>x.mid===cur);if(!m)return;
 ...
 document.getElementById('sPx').textContent=fmt(m.price)+'c';
...
document.getElementById('cmd').onclick=async()=>{
 ...
 if(!confirm((side=='buy'?'Buy ':'Sell ')+sh+' share(s) of '+mid+
   ' at about '+qp.toFixed(2)+' c/share (~'+est.toLocaleString('en-US')+' c)?'))return;
...
def _network_identity(request) -> str | None:
    """Who is calling /api/network/*? Two accepted credentials, during the migration:

      1. `X-Service-Token` matching LEDGER_TOKEN_ESTATES  → identity "estates".
         The scoped path. Same token, same service model as every other core caller,
         so the satellite stops being the one component outside the scope system.
      2. `X-Network-Secret` matching NETWORK_SHARED_SECRET → identity "legacy".
         Kept alive so a satellite that has not been redeployed yet does not break.

    Returns the identity, or None. Once the logs stop showing "legacy" you can delete
    NETWORK_SHARED_SECRET and the second branch with it.

    Both comparisons are constant-time. The old one was a plain `==`, which leaks how
    much of the secret matched through response timing.
    """
    import hmac as _hmac
    tok = str(request.headers.get("X-Service-Token", "") or "").strip()
    if tok:
        try:
            import ledger_v2 as _lv
            for _svc, _known in _lv.load_tokens().items():
                if _svc == "estates" and _hmac.compare_digest(tok, _known):
                    return "estates"
        except Exception:
            _weblog.debug("ledger token check unavailable", exc_info=True)
    try:
        import Restocker_main as _m
        want = str(getattr(_m, "NETWORK_SHARED_SECRET", "") or "")
    except Exception:
        return None
    got = str(request.headers.get("X-Network-Secret", "") or "")
    if want and got and _hmac.compare_digest(got, want):
        return "legacy"
    return None


def _network_secret_ok(request) -> bool:
    """Back-compat wrapper — every existing handler keeps calling this unchanged."""
    who = _network_identity(request)
    if who == "legacy":
        _warn_legacy_network_auth()
...
def _warn_legacy_network_auth() -> None:
    """Say it ONCE per process, not per request — a per-call log on a polling satellite
    would bury everything else."""
    global _legacy_auth_warned
    if not _legacy_auth_warned:
        _legacy_auth_warned = True
        _weblog.warning("[network] a caller is still using the legacy X-Network-Secret. "
                    "Redeploy the satellite with X-Service-Token=LEDGER_TOKEN_ESTATES, "
                    "then remove NETWORK_SHARED_SECRET.")
        print("⚠️  [network] legacy shared-secret auth in use — see log.", flush=True)
...
# ── Land Exchange network API (the "Abex Tech Lands & Auctions" satellite) ──────────
...
async def _land_require_manager(body):
    ...
    (requester_id, None) when the acting user is a Abex Tech manager, else
    ...
    if not rid:
        return "", web.json_response(
            {"ok": False, "error": "requester_id is required — update the Abex Tech Lands & "
    ...
    if not ok:
        return rid, web.json_response(
            {"ok": False, "error": "Managers only — this is decided by your roles on the "
                                   "Abex Tech home server, not on this one."}, status=403)
    return rid, None
...
async def _handle_network_land_cancel(request):
    ...
    # PARTNER guild, so trusting it let any partner-server admin cancel Abex Tech listings.
...
async def start_webserver(port: int = 8080):
    ...
    # ── The core API. ledger_v2 defines register_ledger_routes() but nothing ever
    # called it, so the v2 money API existed in-process (40 call sites use its Python
    # functions) while being unreachable over HTTP. Mounting it is what lets the bank
    # and the estates bot talk to ONE core instead of each having a private back door.
    try:
        import ledger_v2
    ...
    # Loyalty / IGNs / the cross-linked profile. Rides on ledger_v2's service tokens
    # and adds its own read/write scopes — it never touches a money path.
    try:
        import core_profile
