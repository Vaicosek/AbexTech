"""
hub_web.py — the Abex Tech shell + the Markets section.
...
THEME_CSS = r"""
/* Hub-only classes. Everything else — the ground, the type, the sidebar, the
   nav, the tables — comes from `abex_theme` through `abex_shell`; the older
   class vocabulary comes from `vt_web_shell._LEGACY_CSS`, which this appends to
   rather than duplicates. */
/* ---------- hub only ---------- */
/* The front page is a list of what is waiting on you, not a wall of equal
   cards. Each entry is a rule and a heading; the section's own hue marks it
   and nothing else. */
.hub-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));
  gap:34px;margin:34px 0}
.hub-card{display:flex;flex-direction:column;gap:10px;padding:20px 0 0;
  border-top:2px solid var(--hc,var(--accent));background:none;text-decoration:none;
  color:inherit}
...
def money_strip_html(snap: dict, who: str = "") -> str:
    ...
    if not snap.get("ledger_ok"):
        return ('<div class="strip-wrap"><div class="strip">'
                f'<div class="spacer"></div><div class="header-right">{who}</div></div>'
    ...
    # The who-line lives in this row: the strip IS the page header now, and the
...
def page(title: str, active: str, user: Optional[dict], snap: Optional[dict],
    ...
    The chrome itself is `abex_shell` — the one shell in the product. This
    function still owns what is particular to the hub: who is signed in, the
    live money strip, and which sections this deployment actually mounted.
    """
    import abex_shell
    ...
    # A logged-out page (or one with no snapshot) still renders the header row —
    # it carries the sign-in control.
    strip = (money_strip_html(snap, who) if (user and snap) else
             '<div class="strip-wrap"><div class="strip"><div class="spacer"></div>'
             f'<div class="header-right">{who}</div></div></div>')

    staff = _is_staff_user(user)
    mounted = {_NAV_KEY.get(s["key"], s["key"]) for s in sections()
               if staff or not s.get("staff_only")}
    # Where each section is really served. `register_section` has always carried
    # the path; the nav tree carried the design's, and for Markets the two differ
    # (`/exchange`, not `/markets`). The registered one wins — it is the one with
    # a route behind it.
...
def _login_required_page(request: Any) -> Any:
    ...
    return _html(page("Abex Tech — sign in", "", None, None, body), status=401)
...
<form method="post" action="{HUB_PREFIX}/login">
  ...
  <button class="btn" type="submit">Sign in</button>
    ...
    return _html(page("Abex Tech — sign in", "", None, None, body))
...
def _markets_body(uid: str) -> str:
    ...
    out = ['<div class="page-head"><div><h1>Markets</h1>'
           '<div class="page-sub">Shops, stock exchange and hives · Abex Tech core</div></div></div>',
...
async def h_market(request: Any) -> Any:
    ...
    snap = money_snapshot(user["user_id"])
    return _html(page(f"{mid} · Markets · Abex Tech", "markets", user, snap,
...
async def h_market_preview(request: Any) -> Any:
    ...
    return _html(page(f"Confirm · {mid} · Abex Tech", "markets", user, snap,
...
_SECTION_BLURB = {
    "markets":  "Shops, the stock exchange and hives — your Abex Tech core.",
    "banking":  "Abex Bank — savings, credit and bonds.",
...
def _hub_home_body(user: Optional[dict]) -> str:
    ...
    return (
        '<div class="page-head"><div><h1>Abex Tech</h1>'
...
def register_hub_routes(app: Any) -> None:
    ...
    print(f"Abex Tech v{HUB_VERSION} mounted at {HUB_PREFIX}/ "
