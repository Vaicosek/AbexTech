"""
messages_web.py — one-to-one messaging between Abex Tech site users.
...
_CSS = """
<style>
/* Messages — a thread reads like a letter, not a chat app. */
.msg-wrap{display:block}
.msg-empty{padding:6px 0;color:var(--faint);font-size:21px}
.msg-unread{display:inline;padding:0;background:none;color:var(--accent);
  font-family:var(--font-data);font-variant-numeric:tabular-nums;font-size:inherit;
  font-weight:700}
.msg-zero{color:var(--faint)}
.msg-snip{color:var(--text-body);font-family:var(--font-ui);font-size:19px}
.msg-row-new td{background:rgba(201,179,122,.07)}
.msg-thread{border:none;border-top:1px solid var(--border);background:none;
  padding:20px 0 0;max-height:none}
.msg{padding:14px 0;border-bottom:1px solid var(--border)}
.msg:last-child{border-bottom:none}
.msg-h{display:flex;align-items:baseline;gap:12px;margin-bottom:4px}
.msg-who{font-size:21px;font-weight:700;color:var(--text)}
.msg-mine .msg-who{color:var(--accent)}
.msg-when{font-family:var(--font-data);font-size:18px;color:var(--faint)}
.msg-b{font-size:21px;line-height:1.55;color:var(--text);white-space:pre-wrap;
  overflow-wrap:anywhere;max-width:74ch}
.msg-form textarea{width:100%;background:none;border:1px solid var(--border);
  color:var(--text);font-family:var(--font-ui);font-size:21px;line-height:1.5;
  padding:12px 14px;resize:vertical;min-height:110px}
.msg-form textarea:focus{outline:none;border-color:var(--accent)}
.msg-form select{background:var(--bg);border:1px solid var(--border);color:var(--text);
  font-family:var(--font-ui);font-size:19px;padding:8px 10px}
.msg-foot{display:flex;align-items:center;justify-content:space-between;gap:12px;
  margin-top:12px}
.msg-count{font-family:var(--font-data);font-size:18px;color:var(--faint)}
.msg-err{margin-top:9px;font-size:19px;color:var(--red)}
</style>
"""
...
def _thread_body(uid: str, thread: dict, key: str, before: Optional[int],
    ...
    if not _has_wallet(other):
        never = ('<div class="notebox">This player has no wallet on Abex Tech yet. The '
