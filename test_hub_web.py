...
def _install_stubs(db_path: str, calls: Calls, sessions: dict) -> None:
    ...
    w._load_markets = lambda: {MARKET_ID: {"name": "Abex Industries"}}
    w._load_inventory_data = lambda: {"markets": []}
    w._load_stock_data = lambda: {"markets": [{
        "mid": MARKET_ID, "name": "Abex Industries", "ticker": "VTEC",
...
async def test_logged_in_page_renders_real_figures(client, env):
    ...
    assert "Savings and net position need Abex Bank" in body
    ...
    # Theme is served, and there is no emoji anywhere on the page. Warm Feel sets
    # both text and figures in Georgia, so there is no webfont to check for — what
    # matters is that the theme reached the page and that figures are tabular.
